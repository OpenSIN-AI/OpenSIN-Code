...
function App(props: { onSnapshot?: () => Promise<string[]> }) {
  ...
  createEffect(() => {
    ...
    if (route.data.type === "home") {
      renderer.setTerminalTitle("SIN Code")
      return
    }
    ...
    if (route.data.type === "session") {
      ...
      renderer.setTerminalTitle(`SIN | ${title}`)
      return
    }

    if (route.data.type === "plugin") {
      renderer.setTerminalTitle(`SIN | ${route.data.id}`)
    }
  })
  ...
  sdk.event.on("installation.update-available", async (evt) => {
    ...
    await DialogAlert.show(
      dialog,
      "Update Complete",
      `Successfully updated to SIN Code v${result.data.version}. Please restart the application.`,
    )
