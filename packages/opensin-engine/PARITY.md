...
Scope: read-only comparison between the original TypeScript source at `/home/bellman/Workspace/opensin-code/src/` and the Rust port under `rust/crates/`.
...
- Hook config can be inspected via Rust config reporting in `rust/crates/commands/src/lib.rs` and `rust/crates/sin-cli/src/main.rs`.
...
- SIN.md discovery is implemented in `rust/crates/runtime/src/prompt.rs`.
- Rust supports `/memory` and `/init` via `rust/crates/commands/src/lib.rs` and `rust/crates/sin-cli/src/main.rs`.
...
- Main CLI/repl/prompt handling lives in `rust/crates/sin-cli/src/main.rs`.
...
- Stream/tool event translation in `rust/crates/sin-cli/src/main.rs`.
...
### Rust exists
Evidence:
- Core OpenSIN-AI API client in `rust/crates/api/src/{client,error,sse,types}.rs`.
...
- **Prompt mode tools enabled**
  - `rust/crates/sin-cli/src/main.rs` now constructs prompt mode with `LiveCli::new(model, true, ...)`.
- **Default permission mode = DangerFullAccess**
  - Runtime default now resolves to `DangerFullAccess` in `rust/crates/sin-cli/src/main.rs`.
  - Clap default also uses `DangerFullAccess` in `rust/crates/sin-cli/src/args.rs`.
  - Init template writes `dontAsk` in `rust/crates/sin-cli/src/init.rs`.
- **Streaming `{}` tool-input prefix bug**
  - `rust/crates/sin-cli/src/main.rs` now strips the initial empty object only for streaming tool input, while preserving legitimate `{}` in non-stream responses.
