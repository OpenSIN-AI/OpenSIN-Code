...
const CLAUDEAI_SERVER_PREFIX: &str = "opensin.ai ";
...
pub fn scoped_mcp_config_hash(config: &ScopedMcpServerConfig) -> String {
    let rendered = match &config.config {
        ...
        McpServerConfig::ManagedProxy(proxy) => {
            format!("opensinai-proxy|{}|{}", proxy.url, proxy.id)
        }
    };
...
mod tests {
    ...
    fn normalizes_server_names_for_mcp_tooling() {
        ...
        assert_eq!(
            normalize_name_for_mcp("opensin.ai Example   Server!!"),
            "claude_ai_Example_Server"
        );
        assert_eq!(
            mcp_tool_name("opensin.ai Example Server", "weather tool"),
    ...
    fn unwraps_ccr_proxy_urls_for_signature_matching() {
        let wrapped = "https://api.opensin-ai.com/v2/session_ingress/shttp/mcp/123?mcp_url=https%3A%2F%2Fvendor.example%2Fmcp&other=1";
    ...
    fn computes_signatures_for_stdio_and_remote_servers() {
        ...
        let remote = McpServerConfig::Ws(McpWebSocketServerConfig {
            url: "https://api.opensin-ai.com/v2/ccr-sessions/1?mcp_url=wss%3A%2F%2Fvendor.example%2Fmcp".to_string(),
