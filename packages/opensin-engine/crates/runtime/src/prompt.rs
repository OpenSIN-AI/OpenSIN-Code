...
fn discover_instruction_files(cwd: &Path) -> std::io::Result<Vec<ContextFile>> {
    ...
    for dir in directories {
        for candidate in [
            dir.join("SIN.md"),
            dir.join("SIN.local.md"),
            dir.join(".sin").join("SIN.md"),
            dir.join(".sin").join("instructions.md"),
        ] {
...
fn render_project_context(project_context: &ProjectContext) -> String {
    ...
    if !project_context.instruction_files.is_empty() {
        bullets.push(format!(
            "Sin instruction files discovered: {}.",
...
fn render_instruction_files(files: &[ContextFile]) -> String {
    let mut sections = vec!["# Sin instructions".to_string()];
...
mod tests {
    ...
    fn discovers_instruction_files_from_ancestor_chain() {
        ...
        fs::create_dir_all(nested.join(".sin")).expect("nested sin dir");
        fs::write(root.join("SIN.md"), "root instructions").expect("write root instructions");
        fs::write(root.join("SIN.local.md"), "local instructions")
            .expect("write local instructions");
        fs::create_dir_all(root.join("apps")).expect("apps dir");
        fs::create_dir_all(root.join("apps").join(".sin")).expect("apps sin dir");
        fs::write(root.join("apps").join("SIN.md"), "apps instructions")
            .expect("write apps instructions");
        fs::write(
            root.join("apps").join(".sin").join("instructions.md"),
            "apps dot sin instructions",
        )
        .expect("write apps dot sin instructions");
        fs::write(nested.join(".sin").join("SIN.md"), "nested rules")
            .expect("write nested rules");
        fs::write(
            nested.join(".sin").join("instructions.md"),
            "nested instructions",
        )
    ...
    fn dedupes_identical_instruction_content_across_scopes() {
        ...
        fs::write(root.join("SIN.md"), "same rules\n\n").expect("write root");
        fs::write(nested.join("SIN.md"), "same rules\n").expect("write nested");
    ...
    fn displays_context_paths_compactly() {
        assert_eq!(
            display_context_path(Path::new("/tmp/project/.sin/SIN.md")),
            "SIN.md"
        );
    }
    ...
    fn discover_with_git_includes_status_snapshot() {
        ...
        fs::write(root.join("SIN.md"), "rules").expect("write instructions");
    ...
    #[test]
    fn load_system_prompt_reads_sin_files_and_config() {
        let root = temp_dir();
        fs::create_dir_all(root.join(".sin")).expect("sin dir");
        fs::write(root.join("SIN.md"), "Project rules").expect("write instructions");
        fs::write(
            root.join(".sin").join("settings.json"),
        ...
        let original_sin_home = std::env::var("SIN_CONFIG_HOME").ok();
        std::env::set_var("HOME", &root);
        std::env::set_var("SIN_CONFIG_HOME", root.join("missing-home"));
...
",
        ...
        if let Some(value) = original_sin_home {
            std::env::set_var("SIN_CONFIG_HOME", value);
        } else {
            std::env::remove_var("SIN_CONFIG_HOME");
        }
    ...
    #[test]
    fn renders_sin_code_style_sections_with_project_context() {
        let root = temp_dir();
        fs::create_dir_all(root.join(".sin")).expect("sin dir");
        fs::write(root.join("SIN.md"), "Project rules").expect("write SIN.md");
        fs::write(
            root.join(".sin").join("settings.json"),
            r#"{"permissionMode":"acceptEdits"}"#,
        )
        ...
        assert!(prompt.contains("# Sin instructions"));
        assert!(prompt.contains("Project rules"));
        assert!(prompt.contains("permissionMode"));
    ...
    #[test]
    fn discovers_dot_sin_instructions_markdown() {
        let root = temp_dir();
        let nested = root.join("apps").join("api");
        fs::create_dir_all(nested.join(".sin")).expect("nested sin dir");
        fs::write(
        ...
        assert!(context
            .instruction_files
            .iter()
            .any(|file| file.path.ends_with(".sin/instructions.md")));
        assert!(
    ...
    fn renders_instruction_file_metadata() {
        let rendered = render_instruction_files(&[ContextFile {
            path: PathBuf::from("/tmp/project/SIN.md"),
            content: "Project rules".to_string(),
        }]);
        assert!(rendered.contains("# Sin instructions"));
