...
pub const DEFAULT_BASE_URL: &str = "https://api.opensin-ai.com";
...
impl SinApiClient {
    ...
    async fn send_raw_request(
        ...
        let request_builder = self
            .http
            .post(&request_url)
            .header("opensin-ai-version", ANTHROPIC_VERSION)
...
fn resolve_saved_oauth_token_set(
    ...
    let client = SinApiClient::from_auth(AuthSource::None).with_base_url(read_base_url());
...
impl Provider for SinApiClient {
...
mod tests {
    ...
    use super::{
        ...
        resolve_startup_auth_source, AuthSource, SinApiClient, OAuthTokenSet,
    };
    ...
    fn auth_source_from_saved_oauth_when_env_absent() {
        ...
        std::env::set_var("SIN_CONFIG_HOME", &config_home);
        std::env::remove_var("ANTHROPIC_AUTH_TOKEN");
        std::env::remove_var("ANTHROPIC_API_KEY");
        save_oauth_credentials(&runtime::OAuthTokenSet {
    ...
    fn backoff_doubles_until_maximum() {
        let client = SinApiClient::new("test-key").with_retry_policy(
            3,
