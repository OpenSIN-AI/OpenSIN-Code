...
const STARTER_SIN_JSON: &str = concat!(
    "{\n",
    "  \"permissions\": {\n",
    "    \"defaultMode\": \"dontAsk\"\n",
    "  }\n",
...
const GITIGNORE_ENTRIES: [&str; 2] = [".sin/settings.local.json", ".sin/sessions/"];
...
pub(crate) fn initialize_repo(cwd: &Path) -> Result<InitReport, Box<dyn std::error::Error>> {
    ...
    let sin_dir = cwd.join(".sin");
    artifacts.push(InitArtifact {
        name: ".sin/",
        status: ensure_dir(&sin_dir)?,
    });

    let sin_json = cwd.join(".sin.json");
    artifacts.push(InitArtifact {
        name: ".sin.json",
        status: write_file_if_missing(&sin_json, STARTER_SIN_JSON)?,
    });
    ...
    let sin_md = cwd.join("SIN.md");
    let content = render_init_sin_md(cwd);
    artifacts.push(InitArtifact {
        name: "SIN.md",
        status: write_file_if_missing(&sin_md, &content)?,
    });
...
pub(crate) fn render_init_sin_md(cwd: &Path) -> String {
    let detection = detect_repo(cwd);
    let mut lines = vec![
        "# SIN.md".to_string(),
        String::new(),
        "This file provides guidance to OpenSIN Code (sincode.dev) when working with code in this repository.".to_string(),
        String::new(),
    ];
    ...
    lines.push("- Keep shared defaults in `.sin.json`; reserve `.sin/settings.local.json` for machine-local overrides.".to_string());
    lines.push("- Do not overwrite existing `SIN.md` content automatically; update it intentionally when repo workflows change.".to_string());
    lines.push(String::new());
...
#[cfg(test)]
mod tests {
    use super::{initialize_repo, render_init_sin_md};
    use std::fs;
    use std::path::Path;
    ...
    fn temp_dir() -> std::path::PathBuf {
        let nanos = SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir().join(format!("sin-init-{nanos}"))
    }
    ...
    fn initialize_repo_creates_expected_files_and_gitignore_entries() {
        ...
        assert!(rendered.contains(".sin/           created"));
        assert!(rendered.contains(".sin.json       created"));
        assert!(rendered.contains(".gitignore       created"));
        assert!(rendered.contains("SIN.md          created"));
        assert!(root.join(".sin").is_dir());
        assert!(root.join(".sin.json").is_file());
        assert!(root.join("SIN.md").is_file());
        assert_eq!(
            fs::read_to_string(root.join(".sin.json")).expect("read sin json"),
            concat!(
                "{\n",
                "  \"permissions\": {\n",
                "    \"defaultMode\": \"dontAsk\"\n",
        ...
        assert!(gitignore.contains(".sin/settings.local.json"));
        assert!(gitignore.contains(".sin/sessions/"));
        let sin_md = fs::read_to_string(root.join("SIN.md")).expect("read sin md");
        assert!(sin_md.contains("Languages: Rust."));
        assert!(sin_md.contains("cargo clippy --workspace --all-targets -- -D warnings"));
    ...
    fn initialize_repo_is_idempotent_and_preserves_existing_files() {
        ...
        fs::write(root.join("SIN.md"), "custom guidance\n").expect("write existing sin md");
        fs::write(root.join(".gitignore"), ".sin/settings.local.json\n").expect("write gitignore");

        let first = initialize_repo(&root).expect("first init should succeed");
        assert!(first
            .render()
            .contains("SIN.md          skipped (already exists)"));
        let second = initialize_repo(&root).expect("second init should succeed");
        let second_rendered = second.render();
        assert!(second_rendered.contains(".sin/           skipped (already exists)"));
        assert!(second_rendered.contains(".sin.json       skipped (already exists)"));
        assert!(second_rendered.contains(".gitignore       skipped (already exists)"));
        assert!(second_rendered.contains("SIN.md          skipped (already exists)"));
        assert_eq!(
            fs::read_to_string(root.join("SIN.md")).expect("read existing sin md"),
            "custom guidance\n"
        );
        let gitignore = fs::read_to_string(root.join(".gitignore")).expect("read gitignore");
        assert_eq!(gitignore.matches(".sin/settings.local.json").count(), 1);
        assert_eq!(gitignore.matches(".sin/sessions/").count(), 1);
    ...
    fn render_init_template_mentions_detected_python_and_nextjs_markers() {
        ...
        let rendered = render_init_sin_md(Path::new(&root));
