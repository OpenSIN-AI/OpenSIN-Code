...
use api::{
    resolve_startup_auth_source, AuthSource, SinApiClient, ContentBlockDelta, InputContentBlock,
...
fn render_cli_error(problem: &str) -> String {
    ...
    lines.push("  Help             sin --help".to_string());
    lines.join("\n")
}
...
fn format_direct_slash_command_error(command: &str, is_unknown: bool) -> String {
    ...
    if is_unknown {
        ...
        lines.push("  Try              Start `sin` to use interactive slash commands".to_string());
        lines.push(
            "  Tip              Resume-safe commands also work with `sin --resume SESSION.json ...`"
                .to_string(),
        );
    }
    lines.join("\n")
...
fn print_bootstrap_plan() {
    for phase in runtime::BootstrapPlan::sin_default().phases() {
        println!("- {phase:?}");
    }
}
...
fn default_oauth_config() -> OAuthConfig {
    OAuthConfig {
        ...
        authorize_url: String::from("https://platform.sin.dev/oauth/authorize"),
        token_url: String::from("https://platform.sin.dev/v1/oauth/token"),
        callback_port: None,
        manual_redirect_url: None,
        scopes: vec![
            String::from("user:profile"),
            String::from("user:inference"),
            String::from("user:sessions:sin_code"),
        ],
    }
}
...
fn run_login() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Starting Sin OAuth login...");
    ...
    let client = SinApiClient::from_auth(AuthSource::None).with_base_url(api::read_base_url());
    ...
    println!("Sin OAuth login complete.");
    Ok(())
}

fn run_logout() -> Result<(), Box<dyn std::error::Error>> {
    clear_oauth_credentials()?;
    println!("Sin OAuth credentials cleared.");
    Ok(())
}
...
fn wait_for_oauth_callback(
    ...
    let body = if callback.error.is_some() {
        "Sin OAuth login failed. You can close this window."
    } else {
        "Sin OAuth login succeeded. You can close this window."
    };
    let response = format!(
...
impl LiveCli {
    ...
    fn startup_banner(&self) -> String {
        ...
        let has_sin_md = cwd
            .as_ref()
            .is_some_and(|path| path.join("SIN.md").is_file());
        let mut lines = vec![
            format!(
                "{} {}",
                if color {
        ...
        if !has_sin_md {
            lines.push(
                "  First run        /init scaffolds SIN.md, .sin.json, and local session files"
                    .to_string(),
            );
        }
        lines.join("\n")
    ...
    fn run_commit(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let path = write_temp_text_file("sin-commit-message.txt", &message)?;
    ...
    fn run_pr(&self, context: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if command_exists("gh") {
            let body_path = write_temp_text_file("sin-pr-body.md", &body)?;
    ...
    fn run_issue(&self, context: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if command_exists("gh") {
            let body_path = write_temp_text_file("sin-issue-body.md", &body)?;
            let output = Command::new("gh")
...
fn sessions_dir() -> Result<PathBuf, Box<dyn std::error::Error>> {
    let cwd = env::current_dir()?;
    let path = cwd.join(".sin").join("sessions");
...
fn render_memory_report() -> Result<String, Box<dyn std::error::Error>> {
  ...
  Instruction files {}",
    ...
    if project_context.instruction_files.is_empty() {
        ...
        lines.push(
            "  No SIN instruction files discovered in the current directory ancestry.".to_string(),
        );
    } else {
        lines.push("Discovered files".to_string());
...
fn init_sin_md() -> Result<String, Box<dyn std::error::Error>> {
    let cwd = env::current_dir()?;
...
impl DefaultRuntimeClient {
    fn new(
        ...
        Ok(Self {
            ...
            client: SinApiClient::from_auth(resolve_cli_auth_source()?)
...
fn print_help_to(out: &mut impl Write) -> io::Result<()> {
    ...
    writeln!(
        out,
        "  sin                                  Start the interactive REPL"
    )?;
    writeln!(
        out,
        "  sin \"summarize this repo\"            Run one prompt and exit"
    )?;
    writeln!(
        out,
        "  sin prompt \"explain src/main.rs\"     Explicit one-shot prompt"
    )?;
    writeln!(
        out,
        "  sin --resume SESSION.json /status    Inspect a saved session"
    )?;
    writeln!(out)?;
    ...
    writeln!(
        out,
        "  sin dump-manifests                   Read upstream TS sources and print extracted counts"
    )?;
    writeln!(
        out,
        "  sin bootstrap-plan                   Print the bootstrap phase skeleton"
    )?;
    writeln!(
        out,
        "  sin agents                           List configured agents"
    )?;
    writeln!(
        out,
        "  sin skills                           List installed skills"
    )?;
    writeln!(out, "  sin system-prompt [--cwd PATH] [--date YYYY-MM-DD]")?;
    writeln!(
        out,
        "  sin login                            Start the OAuth login flow"
    )?;
    writeln!(
        out,
        "  sin logout                           Clear saved OAuth credentials"
    )?;
    writeln!(
        out,
        "  sin init                             Scaffold SIN.md + local files"
    )?;
    writeln!(out)?;
    writeln!(out, "Flags")?;
    writeln!(
    ...
    writeln!(out, "  sin --model opus \"summarize this repo\"")?;
    writeln!(
        out,
        "  sin --output-format json prompt \"explain src/main.rs\""
    )?;
    writeln!(
        out,
        "  sin --allowedTools read,glob \"summarize Cargo.toml\""
    )?;
    writeln!(
        out,
        "  sin --resume session.json /status /diff /export notes.txt"
    )?;
    writeln!(out, "  sin agents")?;
    writeln!(out, "  sin /skills")?;
    writeln!(out, "  sin login")?;
    writeln!(out, "  sin init")?;
    Ok(())
}
...
mod tests {
    ...
    fn shared_help_uses_resume_annotation_copy() {
        ...
        assert!(help.contains("available via sin --resume SESSION.json"));
    }
    ...
    fn init_help_mentions_direct_subcommand() {
        ...
        assert!(help.contains("sin init"));
        assert!(help.contains("sin agents"));
        assert!(help.contains("sin skills"));
        assert!(help.contains("sin /skills"));
    }
 ...
 M src/main.rs",
    ...
    fn init_template_mentions_detected_rust_workspace() {
        let rendered = crate::init::render_init_sin_md(std::path::Path::new("."));
        assert!(rendered.contains("# SIN.md"));
    ...
    fn ultraplan_progress_lines_include_phase_step_and_elapsed_status() {
        let snapshot = InternalPromptProgressState {
            ...
            detail: Some("reading rust/crates/sin-cli/src/main.rs".to_string()),
            saw_final_text: false,
        };
    ...
    fn describe_tool_progress_summarizes_known_tools() {
        ...
        assert!(
            describe_tool_progress("bash", r#"{"command":"cargo test -p sin-cli"}"#)
                .contains("cargo test -p sin-cli")
        );
        assert_eq!(
