...
fn build_search_url(query: &str) -> Result<reqwest::Url, String> {
    if let Ok(base) = std::env::var("SIN_WEB_SEARCH_BASE_URL") {
...
fn todo_store_path() -> Result<std::path::PathBuf, String> {
    if let Ok(path) = std::env::var("SIN_TODO_STORE") {
        return Ok(std::path::PathBuf::from(path));
    }
    let cwd = std::env::current_dir().map_err(|error| error.to_string())?;
    Ok(cwd.join(".sin-todos.json"))
}
...
fn spawn_agent_job(job: AgentJob) -> Result<(), String> {
    let thread_name = format!("sin-agent-{}", job.manifest.agent_id);
...
fn agent_store_dir() -> Result<std::path::PathBuf, String> {
    if let Ok(path) = std::env::var("SIN_AGENT_STORE") {
        return Ok(std::path::PathBuf::from(path));
    }
    let cwd = std::env::current_dir().map_err(|error| error.to_string())?;
    if let Some(workspace_root) = cwd.ancestors().nth(2) {
        return Ok(workspace_root.join(".sin-agents"));
    }
    Ok(cwd.join(".sin-agents"))
}
...
fn normalize_subagent_type(subagent_type: Option<&str>) -> String {
    ...
    match canonical_tool_token(trimmed).as_str() {
        ...
        "singuide" | "singuideagent" | "guide" => String::from("sin-guide"),
...
fn config_file_for_scope(scope: ConfigScope) -> Result<PathBuf, String> {
    ...
    Ok(match scope {
        ...
        ConfigScope::Settings => cwd.join(".sin").join("settings.local.json"),
    })
}

fn config_home_dir() -> Result<PathBuf, String> {
    if let Ok(path) = std::env::var("SIN_CONFIG_HOME") {
        return Ok(PathBuf::from(path));
    }
    let home = std::env::var("HOME").map_err(|_| String::from("HOME is not set"))?;
    Ok(PathBuf::from(home).join(".sin"))
}
...
mod tests {
    ...
    fn temp_path(name: &str) -> PathBuf {
        let unique = std::time::SystemTime::now()
            ...
            .expect("time")
            .as_nanos();
        std::env::temp_dir().join(format!("sin-tools-{unique}-{name}"))
    }
    ...
    fn web_search_extracts_and_filters_results() {
        ...
        std::env::remove_var("SIN_WEB_SEARCH_BASE_URL");
    ...
    fn web_search_handles_generic_links_and_invalid_base_url() {
        ...
        std::env::set_var("SIN_WEB_SEARCH_BASE_URL", "://bad-base-url");
    ...
    fn todo_write_persists_and_returns_previous_state() {
        ...
        std::env::set_var("SIN_TODO_STORE", &path);
        ...
        .expect("TodoWrite should succeed");
        std::env::remove_var("SIN_TODO_STORE");
    ...
    fn agent_persists_handoff_metadata() {
        ...
        std::env::set_var("SIN_AGENT_STORE", &dir);
        ...
        std::env::remove_var("SIN_AGENT_STORE");
  ...
  "nbformat_minor": 5
    ...
    fn config_reads_and_writes_supported_values() {
        ...
        std::fs::create_dir_all(home.join(".sin")).expect("home dir");
        std::fs::create_dir_all(cwd.join(".sin")).expect("cwd dir");
        std::fs::write(
            home.join(".sin").join("settings.json"),
            r#"{"verbose":false}"#,
        )
        .expect("write global settings");

        let original_home = std::env::var("HOME").ok();
        let original_config_home = std::env::var("SIN_CONFIG_HOME").ok();
        let original_dir = std::env::current_dir().expect("cwd");
        ...
        match original_config_home {
            Some(value) => std::env::set_var("SIN_CONFIG_HOME", value),
            None => std::env::remove_var("SIN_CONFIG_HOME"),
        }
