...
- `sin` CLI for interactive and one-shot coding-agent workflows
...
```bash
cargo install --path crates/sin-cli --locked
# or
cargo build --release -p sin-cli
```
...
```bash
sin
sin prompt "summarize this repository"
```
...
- `sin` binary/package path verified from `cargo metadata`
- CLI command surface verified from `cargo run --quiet --bin sin -- --help`
