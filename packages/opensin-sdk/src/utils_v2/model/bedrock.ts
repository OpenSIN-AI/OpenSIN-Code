...
async function createBedrockClient() {
  ...
  const skipAuth = isEnvTruthy(process.env.OPENSIN_CODE_SKIP_BEDROCK_AUTH)
...
/**
 * Check if a model ID is a foundation model (e.g., "opensin.opensin-sonnet-4-5-20250929-v1:0")
 */
export function isFoundationModel(modelId: string): boolean {
  return modelId.startsWith('opensin.')
}
 ...
 * For example:
 * - "eu.opensin.opensin-sonnet-4-5-20250929-v1:0" → "eu"
 * - "us.opensin.opensin-3-7-sonnet-20250219-v1:0" → "us"
 * - "arn:aws:bedrock:ap-northeast-2:123:inference-profile/global.opensin.opensin-opus-4-6-v1" → "global"
 * - "opensin.opensin-3-5-sonnet-20241022-v2:0" → undefined (foundation model)
 * - "opensin-sonnet-4-5-20250929" → undefined (first-party format)
 */
export function getBedrockRegionPrefix(
  ...
  for (const prefix of BEDROCK_REGION_PREFIXES) {
    if (effectiveModelId.startsWith(`${prefix}.opensin.`)) {
      return prefix
    }
  }
  return undefined
 ...
 * If the model is a foundation model (opensin.*), the prefix will be added.
 * If the model is not a Bedrock model, it will be returned as-is.
 *
 * For example:
 * - applyBedrockRegionPrefix("us.opensin.opensin-sonnet-4-5-v1:0", "eu") → "eu.opensin.opensin-sonnet-4-5-v1:0"
 * - applyBedrockRegionPrefix("opensin.opensin-sonnet-4-5-v1:0", "eu") → "eu.opensin.opensin-sonnet-4-5-v1:0"
 * - applyBedrockRegionPrefix("opensin-sonnet-4-5-20250929", "eu") → "opensin-sonnet-4-5-20250929" (not a Bedrock model)
 */
export function applyBedrockRegionPrefix(
  ...
  // Check if it's a foundation model (opensin.*) and add the prefix
