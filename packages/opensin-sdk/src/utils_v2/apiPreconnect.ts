/**
 * Preconnect to the OpenSIN API to overlap TCP+TLS handshake with startup.
 *
 ...
 * so OPENSIN_BASE_URL/proxy/mTLS in settings would be invisible and preconnect
...
export function preconnectOpenSINApi(): void {
  if (fired) return
  fired = true
  ...
  if (
    ...
    process.env.http_proxy ||
    process.env.OPENSIN_UNIX_SOCKET ||
    process.env.OPENSIN_CODE_CLIENT_CERT ||
    process.env.OPENSIN_CODE_CLIENT_KEY
  ) {
    return
  }

  // Use configured base URL (staging, local, or custom gateway). Covers
  // OPENSIN_BASE_URL env + USE_STAGING_OAUTH + USE_LOCAL_OAUTH in one lookup.
  // NODE_EXTRA_CA_CERTS no longer a skip — init.ts applied it before this fires.
  const baseUrl =
    process.env.OPENSIN_BASE_URL || getOauthConfig().BASE_API_URL
