/**
 * MDM (Mobile Device Management) profile enforcement for OpenSIN Code managed settings.
 *
 * Reads enterprise settings from OS-level MDM configuration:
 * - macOS: `com.opensin.opensincode` preference domain
 *   (MDM profiles at /Library/Managed Preferences/ only — not user-writable ~/Library/Preferences/)
 * - Windows: `HKLM\SOFTWARE\Policies\OpenSINCode` (admin-only)
 *   and `HKCU\SOFTWARE\Policies\OpenSINCode` (user-writable, lowest priority)
 * - Linux: No MDM equivalent (uses /etc/opensin-code/managed-settings.json instead)
 *
