...
// Lazy getters: getOpenSINConfigHomeDir() is memoized and reads process.env.
// Evaluating at module scope would capture the value before entrypoints like
// hfi.tsx get a chance to set OPENSIN_CONFIG_DIR in main(), and would also
// populate the memoize cache with that stale value for all 150+ other callers.
function getLocalInstallDir(): string {
  return join(getOpenSINConfigHomeDir(), 'local')
}
export function getLocalOpenSINPath(): string {
  return join(getLocalInstallDir(), 'opensin')
}
...
export function isRunningFromLocalInstallation(): boolean {
  ...
  return execPath.includes('/.opensin/local/node_modules/')
}
...
export async function ensureLocalPackageEnvironment(): Promise<boolean> {
  try {
    ...
    await writeIfMissing(
      ...
      jsonStringify(
        { name: 'opensin-local', version: '0.0.1', private: true },
        null,
        2,
      ),
    )
    ...
    const wrapperPath = join(localInstallDir, 'opensin')
    const created = await writeIfMissing(
      wrapperPath,
      `#!/bin/sh\nexec "${localInstallDir}/node_modules/.bin/opensin" "$@"`,
      0o755,
    )
    if (created) {
...
/**
 * Install or update OpenSIN CLI package in the local directory
 ...
 */
export async function installOrUpdateOpenSINPackage(
  ...
  try {
    ...
    if (result.code !== 0) {
      const error = new Error(
        `Failed to install OpenSIN CLI package: ${result.stderr}`,
      )
      logError(error)
...
export async function localInstallationExists(): Promise<boolean> {
  try {
    await access(join(getLocalInstallDir(), 'node_modules', '.bin', 'opensin'))
    return true
  } catch {
    return false
  }
