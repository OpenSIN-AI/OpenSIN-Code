 ...
 * user-controlled files (~/.opensin/settings.json and ~/.opensin.json),
 ...
 *
 * We read from global config (~/.opensin.json) and user settings
 * (~/.opensin/settings.json). These are user-controlled files that don't
...
function getExtraCertsPathFromConfig(): string | undefined {
  try {
    ...
    // Only read from user-controlled settings (~/.opensin/settings.json),
