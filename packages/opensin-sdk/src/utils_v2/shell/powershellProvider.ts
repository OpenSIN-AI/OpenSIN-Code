 ...
 * @opensin-ai/sandbox-runtime's shellquote.quote() which would otherwise
  ...
  return {
    ...
    async buildExecCommand(
      ...
      const cwdFilePath =
        opts.useSandbox && opts.sandboxTmpDir
          ? posixJoin(opts.sandboxTmpDir, `opensin-pwd-ps-${opts.id}`)
          : join(tmpdir(), `opensin-pwd-ps-${opts.id}`)
    ...
    async getEnvironmentOverrides(): Promise<Record<string, string>> {
      ...
      if (currentSandboxTmpDir) {
        ...
        env.OPENSIN_CODE_TMPDIR = currentSandboxTmpDir
      }
      return env
    },
  }
