...
export const getMTLSConfig = memoize((): MTLSConfig | undefined => {
  ...
  // Client certificate
  if (process.env.OPENSIN_CODE_CLIENT_CERT) {
    try {
      ...
      logForDebugging(
        'mTLS: Loaded client certificate from OPENSIN_CODE_CLIENT_CERT',
      )
    } catch (error) {
  ...
  // Client key
  if (process.env.OPENSIN_CODE_CLIENT_KEY) {
    try {
      config.key = getFsImplementation().readFileSync(
        process.env.OPENSIN_CODE_CLIENT_KEY,
        { encoding: 'utf8' },
      )
      logForDebugging('mTLS: Loaded client key from OPENSIN_CODE_CLIENT_KEY')
    } catch (error) {
  ...
  // Key passphrase
  if (process.env.OPENSIN_CODE_CLIENT_KEY_PASSPHRASE) {
    config.passphrase = process.env.OPENSIN_CODE_CLIENT_KEY_PASSPHRASE
