...
async function getInstallationPath(): Promise<string> {
  ...
  if (isInBundledMode()) {
    ...
    try {
      const path = await which('opensin')
      if (path) {
        return path
      }
    } catch {
    ...
    try {
      await getFsImplementation().stat(join(homedir(), '.local/bin/opensin'))
      return join(homedir(), '.local/bin/opensin')
    } catch {
      // Not found
    }
    return 'native'
...
> {
  ...
  const localPath = join(homedir(), '.opensin', 'local')
  if (await localInstallationExists()) {
    installations.push({ type: 'npm-local', path: localPath })
  }

  // Check for global npm installation
  const packagesToCheck = ['@opensin-ai/opensin-code']
  ...
  if (npmResult.code === 0 && npmResult.stdout) {
    ...
    // First check for active installations via bin/opensin
    // Linux / macOS have prefix/bin/opensin and prefix/lib/node_modules
    // Windows has prefix/opensin and prefix/node_modules
    const globalBinPath = isWindows
      ? join(npmPrefix, 'opensin')
      : join(npmPrefix, 'bin', 'opensin')
    ...
    if (globalBinExists) {
      ...
      // When npm is installed via Homebrew, both can exist at /opt/homebrew/bin/opensin
      ...
      // If no bin/opensin exists, check for orphaned packages (no bin/opensin symlink)
  ...
  const nativeBinPath = join(homedir(), '.local', 'bin', 'opensin')
  try {
  ...
  if (config.installMethod === 'native') {
    const nativeDataPath = join(homedir(), '.local', 'share', 'opensin')
    try {
...
async function detectConfigurationIssues(
  ...
  if (!isEnvTruthy(process.env.DISABLE_INSTALLATION_CHECKS)) {
    if (type === 'npm-local' && config.installMethod !== 'local') {
      warnings.push({
        ...
        fix: 'Consider using native installation: opensin install',
      })
    }
    ...
    if (type === 'native' && config.installMethod !== 'native') {
      warnings.push({
        ...
        fix: 'Run opensin install to update configuration',
      })
    }
  }
  ...
  const existingAlias = await findOpenSINAlias()
  const validAlias = await findValidOpenSINAlias()

  // Check if running local installation but it's not in PATH
  if (type === 'npm-local') {
    // Check if opensin is already accessible via PATH
    const whichResult = await which('opensin')
    const opensinInPath = !!whichResult

    // Only show warning if opensin is NOT in PATH AND no valid alias exists
    if (!opensinInPath && !validAlias) {
      if (existingAlias) {
        // Alias exists but points to invalid target
        warnings.push({
          issue: 'Local installation not accessible',
          fix: `Alias exists but points to invalid target: ${existingAlias}. Update alias: alias opensin="~/.opensin/local/opensin"`,
        })
      } else {
        // No alias exists and not in PATH
        warnings.push({
          issue: 'Local installation not accessible',
          fix: 'Create alias: alias opensin="~/.opensin/local/opensin"',
        })
      }
    }
  }
  ...
  if (installationType === 'native') {
    ...
    for (const install of npmInstalls) {
      if (install.type === 'npm-global') {
        let uninstallCmd = 'npm -g uninstall @opensin-ai/opensin-code'
        if (
          MACRO.PACKAGE_URL &&
          MACRO.PACKAGE_URL !== '@opensin-ai/opensin-code'
        ) {
  ...
  if (installationType === 'npm-global') {
    ...
    if (!hasUpdatePermissions && !getAutoUpdaterDisabledReason()) {
      warnings.push({
        ...
        fix: 'Do one of: (1) Re-install node without sudo, or (2) Use `opensin install` for native installation',
      })
    }
  }
