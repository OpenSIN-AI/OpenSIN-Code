 ...
 * Terminal Launcher
 *
 * Detects the user's preferred terminal emulator and launches OpenSIN Code
...
/**
 * Launch OpenSIN Code in the detected terminal emulator.
 *
 ...
 *
 * For pure-argv paths: opensinPath, --prefill, query, cwd travel as distinct
 * argv elements end-to-end. No sh -c. No shellQuote(). The terminal does
 * chdir(cwd) and execvp(opensin, argv). Spaces/quotes/metacharacters in
...
export async function launchInTerminal(
  ...
  const opensinArgs = ['--deep-link-origin']
  if (action.repo) {
    opensinArgs.push('--deep-link-repo', action.repo)
    if (action.lastFetchMs !== undefined) {
      opensinArgs.push('--deep-link-last-fetch', String(action.lastFetchMs))
    }
  }
  if (action.query) {
    opensinArgs.push('--prefill', action.query)
  }

  switch (process.platform) {
    case 'darwin':
      return launchMacosTerminal(terminal, opensinPath, opensinArgs, action.cwd)
    case 'linux':
      return launchLinuxTerminal(terminal, opensinPath, opensinArgs, action.cwd)
    case 'win32':
      return launchWindowsTerminal(terminal, opensinPath, opensinArgs, action.cwd)
    default:
      return false
  }
}
...
async function launchMacosTerminal(
  ...
  switch (terminal.command) {
    ...
    case 'iTerm': {
      const shCmd = buildShellCommand(opensinPath, opensinArgs, cwd)
  ...
  activate
    ...
    case 'Ghostty': {
      ...
      args.push('-e', opensinPath, ...opensinArgs)
    ...
    case 'kitty': {
      ...
      args.push(opensinPath, ...opensinArgs)
      const { code } = await execFileNoThrow('open', args, { useCwd: false })
      if (code === 0) return true
      break
    }
...
async function launchLinuxTerminal(
  ...
  switch (terminal.name) {
    ...
    default:
      ...
      args = ['-e', opensinPath, ...opensinArgs]
      spawnCwd = cwd
      break
  }
...
async function launchWindowsTerminal(
  ...
  switch (terminal.name) {
    ...
    // mode that also keeps the session interactive after opensin exits.
    ...
    case 'PowerShell': {
      ...
      args.push(
        '-NoExit',
        '-Command',
        `${cdCmd}& ${psQuote(opensinPath)} ${opensinArgs.map(psQuote).join(' ')}`,
      )
      break
    }
    ...
    default: {
      ...
      args.push(
        '/k',
        `${cdCmd}${cmdQuote(opensinPath)} ${opensinArgs.map(a => cmdQuote(a)).join(' ')}`,
      )
      break
    }
  }
...
function buildShellCommand(
  ...
  return `${cdPrefix}${[opensinPath, ...opensinArgs].map(shellQuote).join(' ')}`
}
 ...
 * *child process* (opensin.exe) uses CommandLineToArgvW, where a trailing
