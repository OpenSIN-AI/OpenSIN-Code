 ...
 * opensin-cli:// deep link. Linux xdg-open and browsers with "always allow"
 * set dispatch the link with no OS-level confirmation, so the application
 * provides its own provenance signal — mirroring opensin.ai's security
 ...
 * notice which directory — and therefore which OPENSIN.md — was loaded.
 */
 ...
 *
 * Always shows the working directory so the user can see which OPENSIN.md
 ...
 * and whether its OPENSIN.md may be stale relative to upstream.
 */
export function buildDeepLinkBanner(info: DeepLinkBannerInfo): string {
  ...
  if (info.repo) {
    ...
    lines.push(
      `Resolved ${info.repo} from local clones · last fetched ${age}${stale ? ' — OPENSIN.md may be stale' : ''}`,
    )
  }
