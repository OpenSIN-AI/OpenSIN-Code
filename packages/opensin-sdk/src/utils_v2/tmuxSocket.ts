 ...
 * =====================
 * This module manages an isolated tmux socket for OpenSIN's operations.
 *
 * WHY THIS EXISTS:
 * Without isolation, OpenSIN could accidentally affect the user's tmux sessions.
 * For example, running `tmux kill-session` via the Bash tool would kill the
 * user's current session if they started OpenSIN from within tmux.
 *
 * HOW IT WORKS:
 * 1. OpenSIN creates its own tmux socket: `opensin-<PID>` (e.g., `opensin-12345`)
 ...
 *    (set in Shell.ts via getOpenSINTmuxEnv())
 *
 * This means ANY tmux command run through OpenSIN - whether via the Tmux tool
 * directly or via Bash - will operate on OpenSIN's isolated socket, NOT the
 * user's tmux session.
 *
 * IMPORTANT: The user's original TMUX env var is NOT used. After socket
 * initialization, getOpenSINTmuxEnv() returns a value that overrides the
...
const OPENSIN_SOCKET_PREFIX = 'opensin'
...
/**
 * Gets the socket name for OpenSIN's isolated tmux session.
 * Format: opensin-<PID>
 */
export function getOpenSINSocketName(): string {
  if (!socketName) {
    socketName = `${OPENSIN_SOCKET_PREFIX}-${process.pid}`
  }
  return socketName
}
 ...
 */
export function getOpenSINSocketPath(): string | null {
  return socketPath
}
 ...
 */
export function setOpenSINSocketInfo(path: string, pid: number): void {
  socketPath = path
  serverPid = pid
}
...
/**
 * Gets the TMUX environment variable value for OpenSIN's isolated socket.
 *
 ...
 * initializing the socket. getOpenSINTmuxEnv() will return null, and
 * Bash commands will run without tmux isolation.
 */
...
async function doInitialize(): Promise<void> {
  ...
  // Pass OPENSIN_CODE_SKIP_PROMPT_HISTORY via -e so it's set in the initial shell environment
  //
  ...
  const result = await execTmux([
    ...
    'base',
    '-e',
    'OPENSIN_CODE_SKIP_PROMPT_HISTORY=true',
    ...(getPlatform() === 'windows'
  ...
  // Set OPENSIN_CODE_SKIP_PROMPT_HISTORY in the tmux GLOBAL environment (-g).
  ...
  // Any OpenSIN Code instance spawned on this socket will inherit this env var,
  ...
  await execTmux([
    ...
    'set-environment',
    '-g',
    'OPENSIN_CODE_SKIP_PROMPT_HISTORY',
    'true',
  ])
  ...
  if (pidResult.code === 0) {
    ...
    if (!isNaN(pid)) {
      ...
      setOpenSINSocketInfo(fallbackPath, pid)
      return
    }
    // PID parsing failed
    logForDebugging(
