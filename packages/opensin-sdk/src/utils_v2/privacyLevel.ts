 ...
 *   OPENSIN_CODE_DISABLE_NONESSENTIAL_TRAFFIC  →  essential-traffic
 ...
 * Equivalent to the old `process.env.OPENSIN_CODE_DISABLE_NONESSENTIAL_TRAFFIC` check.
 */
...
export function getEssentialTrafficOnlyReason(): string | null {
  if (process.env.OPENSIN_CODE_DISABLE_NONESSENTIAL_TRAFFIC) {
    return 'OPENSIN_CODE_DISABLE_NONESSENTIAL_TRAFFIC'
  }
  return null
}
