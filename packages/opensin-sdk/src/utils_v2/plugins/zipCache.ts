 ...
 * When OPENSIN_CODE_PLUGIN_USE_ZIP_CACHE is enabled and OPENSIN_CODE_PLUGIN_CACHE_DIR
...
export function isPluginZipCacheEnabled(): boolean {
  return isEnvTruthy(process.env.OPENSIN_CODE_PLUGIN_USE_ZIP_CACHE)
}

/**
 * Get the path to the zip cache directory.
 * Requires OPENSIN_CODE_PLUGIN_CACHE_DIR to be set.
...
export function getPluginZipCachePath(): string | undefined {
  ...
  const dir = process.env.OPENSIN_CODE_PLUGIN_CACHE_DIR
...
export async function getSessionPluginCachePath(): Promise<string> {
  ...
  if (!sessionPluginCachePromise) {
    sessionPluginCachePromise = (async () => {
      ...
      const dir = join(tmpdir(), `opensin-plugin-session-${suffix}`)
...
async function collectFilesForZip(
  ...
  // See also: markdownConfigLoader.ts getFileIdentity, opensins/opensin-code#13893
  try {
