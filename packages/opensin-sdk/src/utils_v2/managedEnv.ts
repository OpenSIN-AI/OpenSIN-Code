...
/**
 * `opensin ssh` remote: OPENSIN_UNIX_SOCKET routes auth through a -R forwarded
 * socket to a local proxy, and the launcher sets a handful of placeholder auth
 * env vars that the remote's ~/.opensin settings.env MUST NOT clobber (see
 * isOpenSINAuthEnabled). Strip them from any settings-sourced env object.
 */
function withoutSSHTunnelVars(
  env: Record<string, string> | undefined,
): Record<string, string> {
  if (!env || !process.env.OPENSIN_UNIX_SOCKET) return env || {}
  const {
    OPENSIN_UNIX_SOCKET: _1,
    OPENSIN_BASE_URL: _2,
    OPENSIN_API_KEY: _3,
    OPENSIN_AUTH_TOKEN: _4,
    OPENSIN_CODE_OAUTH_TOKEN: _5,
    ...rest
  } = env
  return rest
}
 ...
 * OPENSIN_CODE_PROVIDER_MANAGED_BY_HOST in spawn env), strip
 * provider-selection / model-default vars from settings-sourced env so a
 * user's ~/.opensin/settings.json can't redirect requests away from the
...
function withoutHostManagedProviderVars(
  ...
  if (!env) return {}
  if (!isEnvTruthy(process.env.OPENSIN_CODE_PROVIDER_MANAGED_BY_HOST)) {
    return env
  }
 ...
 *
 * - userSettings (~/.opensin/settings.json): controlled by the user, not project-specific
 ...
 * traffic (e.g., OPENSIN_BASE_URL) to an attacker-controlled server.
 */
 ...
 * OPENSIN_BASE_URL take effect during first-run/onboarding.
 *
 * For trusted sources (user settings, managed settings, CLI flags), ALL env vars
 * are applied — including ones like OPENSIN_BASE_URL that would be dangerous
...
export function applySafeConfigEnvironmentVariables(): void {
  ...
  // Global config (~/.opensin.json) is user-controlled. In CCD mode,
  ...
  // doesn't get clobbered by ~/.opensin/settings.json env (gh#217). policy/flag
  ...
  // flagSettings env applied. Eligibility reads OPENSIN_CODE_USE_BEDROCK,
  // OPENSIN_BASE_URL — both settable via settings.env.
  ...
  // when OPENSIN_CODE_PROVIDER_MANAGED_BY_HOST is set.
 ...
 * OPENSIN_CODE_PROVIDER_MANAGED_BY_HOST is set — see filterSettingsEnv) and
