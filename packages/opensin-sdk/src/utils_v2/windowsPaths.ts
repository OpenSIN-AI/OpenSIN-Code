...
export const findGitBashPath = memoize((): string => {
  if (process.env.OPENSIN_CODE_GIT_BASH_PATH) {
    if (checkPathExists(process.env.OPENSIN_CODE_GIT_BASH_PATH)) {
      return process.env.OPENSIN_CODE_GIT_BASH_PATH
    }
    // biome-ignore lint/suspicious/noConsole:: intentional console output
    console.error(
      `OpenSIN Code was unable to find OPENSIN_CODE_GIT_BASH_PATH path "${process.env.OPENSIN_CODE_GIT_BASH_PATH}"`,
    )
  ...
  // biome-ignore lint/suspicious/noConsole:: intentional console output
  console.error(
    'OpenSIN Code on Windows requires git-bash (https://git-scm.com/downloads/win). If installed but not in PATH, set environment variable pointing to your bash.exe, similar to: OPENSIN_CODE_GIT_BASH_PATH=C:\\Program Files\\Git\\bin\\bash.exe',
  )
  // eslint-disable-next-line custom-rules/no-process-exit
  process.exit(1)
})
