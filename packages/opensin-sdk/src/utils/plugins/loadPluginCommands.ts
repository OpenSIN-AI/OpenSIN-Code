...
function createPluginCommand(
  ...
  try {
    ...
    // Substitute ${SIN_PLUGIN_ROOT} in allowed-tools before parsing
    ...
    return {
      ...
      async getPromptForCommand(args, context) {
        ...
        // Replace ${SIN_PLUGIN_ROOT} and ${SIN_PLUGIN_DATA} with their paths
        ...
        // Replace ${SIN_SKILL_DIR} with this specific skill's directory.
        // Distinct from ${SIN_PLUGIN_ROOT}: a plugin can contain multiple
        // skills, so SIN_PLUGIN_ROOT points to the plugin root while
        // SIN_SKILL_DIR points to the individual skill's subdirectory.
        ...
        // Replace ${SIN_SESSION_ID} with the current session ID
        finalContent = finalContent.replace(
          /\$\{SIN_SESSION_ID\}/g,
          getSessionId(),
        )
