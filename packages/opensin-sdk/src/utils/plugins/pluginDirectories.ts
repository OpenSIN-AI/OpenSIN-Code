...
import { getSinConfigHomeDir, isEnvTruthy } from '../envUtils.js'
 ...
 * 2. Default: ~/.sin/plugins or ~/.sin/cowork_plugins
 */
export function getPluginsDirectory(): string {
  ...
  // like "~/.sin/plugins" becomes a literal `~` directory created in the
  ...
  return join(getSinConfigHomeDir(), getPluginsDirectoryName())
}
 ...
 * ${SIN_PLUGIN_DATA}. Unlike the version-scoped install cache
 * (${SIN_PLUGIN_ROOT}, which is orphaned and GC'd on every update),
 ...
 * .replace() so this isn't invoked unless ${SIN_PLUGIN_DATA} is present
