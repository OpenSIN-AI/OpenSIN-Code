 ...
 * one into the other. Surfaced as warnings by `sin plugin validate` since
...
function detectManifestType(
  ...
  // Check if it's in .sin-plugin directory
  if (dirName === '.sin-plugin') {
 ...
 * '..' a user added to "climb out of .sin-plugin/" is unnecessary. Callers pass `hint`
...
// they expect paths to resolve relative to marketplace.json (inside .sin-plugin/),
...
function marketplaceSourceHint(p: string): string {
  ...
  // .sin-plugin/" is unnecessary since paths already start at the repo root.
  ...
  return (
    ...
    'containing .sin-plugin/), not relative to marketplace.json. ' +
...
export async function validatePluginManifest(
  ...
  // them. `sin plugin validate` is a developer tool — authors running it
  ...
  if (result.success) {
    ...
    // but the Sin.ai marketplace sync rejects non-kebab names. Surfacing
    ...
    if (!/^[a-z0-9]+(-[a-z0-9]+)*$/.test(manifest.name)) {
      warnings.push({
        ...
        message:
          ...
          `it, but the Sin.ai marketplace sync requires kebab-case ` +
  ...
  if (result.success) {
    ...
    if (marketplace.plugins) {
      ...
      const marketplaceRoot =
        path.basename(manifestDir) === '.sin-plugin'
      ...
      for (const [i, entry] of marketplace.plugins.entries()) {
        ...
        if (manifestVersion && manifestVersion !== entry.version) {
          warnings.push({
            ...
            message:
              `Entry declares version "${entry.version}" but ${entry.source}/.sin-plugin/plugin.json says "${manifestVersion}". ` +
 ...
 * for the load path, but authors running `sin plugin validate` want a hard
...
function validateComponentFile(
  ...
  if (fm.description !== undefined) {
    ...
    warnings.push({
      path: 'description',
      message:
        `No description in frontmatter. A description helps users and Sin ` +
...
export async function validateManifest(
  ...
  if (stats?.isDirectory()) {
    // Look for manifest files in .sin-plugin directory
    ...
    const pluginPath = path.join(absolutePath, '.sin-plugin', 'plugin.json')
    ...
    return {
      ...
      errors: [
        {
          path: 'directory',
          message: `No manifest found in directory. Expected .sin-plugin/marketplace.json or .sin-plugin/plugin.json`,
        },
      ],
      warnings: [],
      filePath: absolutePath,
