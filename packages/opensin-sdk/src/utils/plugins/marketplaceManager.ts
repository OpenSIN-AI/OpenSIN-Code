...
export function getDeclaredMarketplaces(): Record<string, DeclaredMarketplace> {
  ...
  // An explicit extraKnownMarketplaces entry for sin-plugins-official
 ...
 *
 * Reads the configuration file at ~/.sin/plugins/known_marketplaces.json
 ...
 *     "installLocation": "/Users/me/.sin/plugins/marketplaces/official-marketplace.json",
 *     "lastUpdated": "2024-01-15T10:30:00.000Z"
 *   },
 *   "company-plugins": {
 *     "source": { "source": "github", "repo": "mycompany/plugins" },
 *     "installLocation": "/Users/me/.sin/plugins/marketplaces/company-plugins",
 ...
 *
 * Writes the configuration to ~/.sin/plugins/known_marketplaces.json,
...
async function cacheMarketplaceFromUrl(
  ...
  const headers = {
    ...
    'User-Agent': 'Sin-Code-Plugin-Manager',
  }
 ...
 * - GitHub: Clones repo and looks for .sin-plugin/marketplace.json
    ...
    switch (source.source) {
      ...
      case 'github': {
        ...
        marketplacePath = join(
          temporaryCachePath,
          source.path || '.sin-plugin/marketplace.json',
        )
        break
      }
      ...
      case 'file': {
        ...
        // File sources point to .sin-plugin/marketplace.json, so the marketplace
        // root is two directories up (parent of .sin-plugin/)
      ...
      case 'directory': {
        // For directories, look for .sin-plugin/marketplace.json
        // Resolve to absolute so error messages show the actual path checked
        // (legacy known_marketplaces.json entries may have relative paths)
        const absPath = resolve(source.path)
        marketplacePath = join(absPath, '.sin-plugin', 'marketplace.json')
 ...
 * The configuration is saved to ~/.sin/plugins/known_marketplaces.json.
 *
  ...
  if (seedDir) {
    throw new Error(
      `Marketplace '${name}' is registered from the read-only seed directory ` +
        ...
        `To stop using its plugins: sin plugin disable <plugin>@${name}`,
    )
  }
...
async function readCachedMarketplace(
  ...
  // For git-sourced directories, the manifest lives at .sin-plugin/marketplace.json.
  ...
  const nestedPath = join(installLocation, '.sin-plugin', 'marketplace.json')
  try {
    ...
    if (
      ...
      throw new Error(
        `Marketplace "${name}" has a relative source path (${entry.source.path}) ` +
          ...
          `OpenSIN Code version. Run 'sin marketplace remove ${name}' and ` +
...
export async function refreshAllMarketplaces(): Promise<void> {
  ...
  for (const [name, entry] of Object.entries(config)) {
    ...
    // hits this path on `sin plugin marketplace update` (no name arg).
      ...
      if (resolvedLoc !== cacheDir && !resolvedLoc.startsWith(cacheDir + sep)) {
        throw new Error(
          `Marketplace '${name}' has a corrupted installLocation ` +
            ...
            `Run: sin plugin marketplace remove "${name}" and re-add it.`,
        )
      }
    }
      ...
      try {
        ...
        const reason =
          name === 'opensin-code-plugins'
            ? `We've deprecated "opensin-code-plugins" in favor of "sin-plugins-official".`
        ...
        throw new Error(
          `The marketplace.json file is no longer present in this repository.\n\n` +
            ...
            `You can remove this marketplace with: sin plugin marketplace remove "${name}"`,
        )
      }
