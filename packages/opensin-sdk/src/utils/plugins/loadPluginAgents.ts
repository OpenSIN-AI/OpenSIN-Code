...
async function loadAgentFromFile(
  ...
  try {
    ...
    // Substitute ${SIN_PLUGIN_ROOT} so agents can reference bundled files,
    ...
    // time. For this level of control, define the agent in .sin/agents/
    ...
    for (const field of ['permissionMode', 'hooks', 'mcpServers'] as const) {
      if (frontmatter[field] !== undefined) {
        logForDebugging(
          `Plugin agent file ${filePath} sets ${field}, which is ignored for plugin agents. Use .sin/agents/ for this level of control.`,
          { level: 'warn' },
        )
      }
    }
