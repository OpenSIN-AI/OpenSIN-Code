 ...
 * Handles ${SIN_PLUGIN_ROOT}, ${user_config.X}, and general ${VAR} substitution
...
export function resolvePluginMcpEnvironment(
  ...
  switch (config.type) {
    ...
    case 'stdio': {
      ...
      // Resolve environment variables and add SIN_PLUGIN_ROOT / SIN_PLUGIN_DATA
      const resolvedEnv: Record<string, string> = {
        SIN_PLUGIN_ROOT: plugin.path,
        SIN_PLUGIN_DATA: getPluginDataDir(plugin.source),
        ...(stdioConfig.env || {}),
      }
      for (const [key, value] of Object.entries(resolvedEnv)) {
        if (key !== 'SIN_PLUGIN_ROOT' && key !== 'SIN_PLUGIN_DATA') {
    ...
    // For other types (sse-ide, ws-ide, sdk, sinai-proxy), pass through unchanged
    case 'sse-ide':
    case 'ws-ide':
    case 'sdk':
    case 'sinai-proxy':
