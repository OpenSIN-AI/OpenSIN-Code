 ...
 *
 * State persists in GlobalConfig.sinCodeHints — a show-once record per
...
/**
 * Hard cap on `sinCodeHints.plugin[]` — bounds config growth. Each shown
...
export function markHintPluginShown(pluginId: string): void {
  saveGlobalConfig(current => {
    const existing = current.sinCodeHints?.plugin ?? []
    if (existing.includes(pluginId)) return current
    return {
      ...current,
      sinCodeHints: {
        ...current.sinCodeHints,
...
export function disableHintRecommendations(): void {
  saveGlobalConfig(current => {
    if (current.sinCodeHints?.disabled) return current
    return {
      ...current,
      sinCodeHints: { ...current.sinCodeHints, disabled: true },
    }
  })
}
