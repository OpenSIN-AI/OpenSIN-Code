...
export function saveMcpServerUserConfig(
  ...
  try {
    ...
    if (
      ...
      // pluginOptionsStorage.ts:184, see SIN.md's 10% case).
...
async function generateMcpConfig(
  ...
  // Lazy import: @opensin-ai-ai/mcpb barrel pulls in zod v3 schemas (~700KB of
  // bound closures). See dxt/helpers.ts for details.
  const { getMcpConfigForManifest } = await import('@opensin-ai-ai/mcpb')
