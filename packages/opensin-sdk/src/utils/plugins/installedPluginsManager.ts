 ...
 *
 * The enabled/disabled state remains in .sin/settings.json for per-repo control.
 *
 ...
 *
 * Legacy cache structure: ~/.sin/plugins/cache/{plugin-name}/
 * Versioned cache structure: ~/.sin/plugins/cache/{marketplace}/{plugin}/{version}/
 *
...
function migrateV1ToV2(v1Data: InstalledPluginsFileV1): InstalledPluginsFileV2 {
  ...
  for (const [pluginId, plugin] of Object.entries(v1Data.plugins)) {
    // V2 format uses versioned cache path: ~/.sin/plugins/cache/{marketplace}/{plugin}/{version}
 ...
 *
 * getOriginalCwd() (not getCwd()) because "current project" is where Sin
...
function getPluginVersionFromManifest(
  ...
  const fs = getFsImplementation()
  const manifestPath = join(pluginCachePath, '.sin-plugin', 'plugin.json')
    ...
    if (existingInstallations && existingInstallations.length > 0) {
      ...
      try {
        ...
        if (typeof entry.source === 'string') {
          ...
          // Only read manifest if the .sin-plugin dir is present
          if (dirEntries.includes('.sin-plugin')) {
