 ...
 * 1. Caching a plugin to ~/.sin/plugins/cache/
  ...
  if (cacheResult.path !== versionedPath) {
    ...
    if (isSubdirectory) {
      ...
      const tempPath = join(
        ...
        `.sin-plugin-temp-${Date.now()}-${randomBytes(4).toString('hex')}`,
      )
...
export async function installPluginFromMarketplace({
  ...
  try {
    ...
    // non-official) because dbt external_sin_code_plugin_installs.sql
