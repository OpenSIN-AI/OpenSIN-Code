 ...
 * Format: ~/.sin/plugins/cache/{marketplace}/{plugin}/{version}/
 *
 * @param pluginId - Plugin identifier in format "name@marketplace"
 * @param version - Version string (semver, git SHA, etc.)
 ...
 * Format: ~/.sin/plugins/cache/{plugin-name}/
 *
...
export async function cachePlugin(
  ...
  const manifestPath = join(tempPath, '.sin-plugin', 'plugin.json')
 ...
 * @param source - Source description for default manifest (e.g., "git:repo" or ".sin-plugin/name")
 ...
 * @param source - Source identifier (e.g., "git:repo", ".sin-plugin/my-plugin")
...
export async function createPluginFromPath(
  ...
  const manifestPath = join(pluginPath, '.sin-plugin', 'plugin.json')
  ...
  const plugin: LoadedPlugin = {
    ...
    source, // Source identifier (e.g., "git:repo" or ".sin-plugin/name")
 ...
 * main.tsx's getSinCodeMcpConfigs()/getInitialSettings().agent run
 ...
 * - After modifying .sin-plugin/ directory (for export)
