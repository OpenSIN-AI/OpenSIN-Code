...
describe("BackgroundAgentManager — Lifecycle", () => {
  ...
  it("cannot transition from terminal to non-terminal", async () => {
    ...
    const updated = manager.findById(agent.id);
    expect(updated!.result).toBe("Done"); // Original result preserves
  });
});
...
describe("BackgroundAgentManager — Result Retrieval", () => {
  ...
  it("returns waiting message for running agent", async () => {
    // Create a manager with very short terminal wait grace period
    const fastManager = new BackgroundAgentManager({
      baseDir: tempDir,
      terminalWaitGraceMs: 50,
    });

    const agent = await fastManager.spawn(createSpawnInput());
    const result = await fastManager.getResult(agent.id, undefined);
