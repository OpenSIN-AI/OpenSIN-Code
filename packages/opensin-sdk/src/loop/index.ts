...
export class LoopMode {
  ...
  async startLoop(loopId: string): Promise<LoopState> {
    ...
    this.runLoopIteration(loopId).catch((err) => {
      state.lastError = err instanceof Error ? err.message : String(err);
      this.stopLoop(loopId, "unhandled_error");
    });
    return state;
  }
  ...
  private async runLoopIteration(loopId: string): Promise<void> {
    ...
    try {
      ...
      if (errorCondition) {
        const maxErrors = typeof errorCondition.value === "number" ? errorCondition.value : 1;
        if ((state.errorCount || 0) >= maxErrors) {
  ...
  private aggregateResult(loopId: string, reason: string): void {
    ...
    const result: LoopResult = {
      ...
      stopReason: reason,
      aggregatedResult: `Loop completed ${state.currentIteration} iterations. Stopped: ${reason}`,
