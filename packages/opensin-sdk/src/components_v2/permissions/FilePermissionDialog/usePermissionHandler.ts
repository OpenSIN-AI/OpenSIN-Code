...
export type PermissionHandlerOptions = {
  ...
  scope?: 'opensin-folder' | 'global-opensin-folder'
}
...
function handleAcceptSession(
  ...
  // For opensin-folder scope, grant session-level access to all .opensin/ files
  if (
    options?.scope === 'opensin-folder' ||
    options?.scope === 'global-opensin-folder'
  ) {
    const pattern =
      options.scope === 'global-opensin-folder'
        ? GLOBAL_OPENSIN_FOLDER_PERMISSION_PATTERN
