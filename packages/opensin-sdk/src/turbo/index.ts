import { AuditEntry, CommandResult, CommandSafetyFilter, TurboConfig } from "./models.js";

export { AuditEntry, CommandResult, CommandSafetyFilter, TurboConfig } from "./models.js";
...
function createDefaultFilter(): CommandSafetyFilter {
  return {
...
export class CommandSafetyFilter {
  private config: CommandSafetyFilter;

  constructor(config?: Partial<CommandSafetyFilter>) {
  ...
  updateConfig(updates: Partial<CommandSafetyFilter>): void {
    this.config = { ...this.config, ...updates };
  }

  getConfig(): CommandSafetyFilter {
    return { ...this.config };
  }
}
...
export class TurboMode {
  ...
  private safetyFilter: CommandSafetyFilter;
  private auditTrail: AuditTrail;

  constructor() {
    this.safetyFilter = new CommandSafetyFilter();
  ...
  checkCommand(command: string): { allowed: boolean; reason: string } {
    if (!this.config.enabled) {
      return { allowed: false, reason: "Turbo mode is disabled" };
    }
  ...
  updateSafetyFilter(updates: Partial<CommandSafetyFilter>): void {
...
export const turboMode = new TurboMode();
