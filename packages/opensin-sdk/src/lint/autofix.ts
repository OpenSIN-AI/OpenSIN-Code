...
export class AutoFixEngine {
  ...
  async fixAll(files?: string[]): Promise<{ results: LintResult[]; fixed: LintError[]; failed: LintError[] }> {
    const targetFiles = files && files.length > 0 ? files : [files?.[0]];
    const allResults: LintResult[] = [];
    const allFixed: LintError[] = [];
    const allFailed: LintError[] = [];
    ...
    return { results: allResults, fixed: allFixed, failed: allFailed };
  }
