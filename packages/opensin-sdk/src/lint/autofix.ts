...
export class AutoFixEngine {
  ...
  async #attemptFix(error: LintError): Promise<FixAttempt> {
  ...
  async #attemptAutoFix(error: LintError, file: string): Promise<FixAttempt> {
    ...
    try {
      ...
      switch (source) {
        case "eslint": {
          ...
          await execAsync(cmd, { cwd: process.cwd(), timeout: this.#config.timeout })
          break
        }
        case "prettier": {
  ...
  #applyFixToContent(content: string, fix: LintFix): string {
    const [start, end] = fix.range
    return content.slice(0, start) + fix.text + content.slice(end)
  }

  #filterErrors(errors: LintError[]): LintError[] {
    return errors.filter((error) => {
      if (!this.#config.sources.includes(error.source)) {
        return false
      }

      const ignoreRules = this.#config.ignoreRules ?? []
      const onlyRules = this.#config.onlyRules ?? []
