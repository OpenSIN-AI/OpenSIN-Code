...
import type { LintConfig, LintError, LintResult, LintSource, LintFix } from "./types.js"

const execAsync = promisify(exec)

let errorIdCounter = 0

function generateId(source: LintSource): string {
  return `${source}_${++errorIdCounter}_${Date.now()}`
}

function mapSeverity(severity: string): "error" | "warning" | "info" {
  const lower = severity.toLowerCase()
  if (lower === "error" || lower === "e") return "error"
  if (lower === "warning" || lower === "warn" || lower === "w") return "warning"
  return "info"
}
...
export class LinterAggregator {
  ...
  async lint(file?: string): Promise<LintResult[]> {
    ...
    const tasks: Promise<LintResult>[] = []
  ...
  async #lintESLint(file?: string): Promise<LintResult> {
    ...
    try {
      ...
      const output = JSON.parse(stdout) as ESLintFileResult[]

      for (const result of output ?? []) {
        for (const msg of result.messages ?? []) {
          const fix: LintFix | undefined = msg.fix ? { range: msg.fix.range, text: msg.fix.text } : undefined
  ...
  async #lintPylint(file?: string): Promise<LintResult> {
    const start = Date.now()
    const errors: LintError[] = []
    const warnings: LintError[] = []
  ...
  async #lintPrettier(file?: string): Promise<LintResult> {
    const start = Date.now()
    const errors: LintError[] = []
  ...
  async #lintTSC(file?: string): Promise<LintResult> {
    const start = Date.now()
    const errors: LintError[] = []
    const warnings: LintError[] = []
