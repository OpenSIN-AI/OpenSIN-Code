...
export function registerBuiltinHooks(
  ...
  if (prettierOpts.enabled !== false) {
    hooks.push({
      ...
      command: "npx",
      args: ["prettier", "--write", ...(prettierOpts.args ?? [process.env['OPENSIN_HOOK_FILE'] ?? "."])],
  ...
  if (eslintOpts.enabled !== false) {
    hooks.push({
      ...
      event: HookEvent.PostEdit,
      command: "npx",
      args: ["eslint", "--fix", ...(eslintOpts.args ?? [process.env['OPENSIN_HOOK_FILE'] ?? "."])],
  ...
  if (autoLintOpts.enabled !== false) {
    hooks.push({
      ...
      event: HookEvent.PostEdit,
      command: "npx",
      args: ["--no", "opensin", "lint", "--fix", process.env['OPENSIN_HOOK_FILE'] ?? "."],
