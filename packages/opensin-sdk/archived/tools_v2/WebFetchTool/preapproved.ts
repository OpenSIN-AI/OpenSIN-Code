...
// for the handful of path-scoped entries (e.g., "github.com/opensins").
...
export function isPreapprovedHost(hostname: string, pathname: string): boolean {
  ...
  if (prefixes) {
    for (const p of prefixes) {
      // Enforce path segment boundaries: "/opensins" must not match
      // "/opensins-evil/malware". Only exact match or a "/" after the
      // prefix is allowed.
