 ...
 * via shell expansion (e.g., ${OPENSIN_API_KEY}) in Bash tool commands.
 *
 * The parent opensin process keeps these vars (needed for API calls, lazy
...
const GHA_SUBPROCESS_SCRUB = [
  // OpenSIN auth — opensin re-reads these per-request, subprocesses don't need them
  'OPENSIN_API_KEY',
  ...
  // GitHub Actions OIDC — consumed by the action's JS before opensin spawns;
  ...
  // opensin-code-action-specific duplicates — action JS consumes these during
  // prepare, before spawning opensin. ALL_INPUTS contains opensin_api_key as JSON.
  'ALL_INPUTS',
 ...
 * servers, shell hooks).
 *
 * Gated on OPENSIN_CODE_SUBPROCESS_ENV_SCRUB. opensin-code-action sets this
...
export function subprocessEnv(): NodeJS.ProcessEnv {
  ...
  if (!isEnvTruthy(process.env.OPENSIN_CODE_SUBPROCESS_ENV_SCRUB)) {
  ...
  for (const k of GHA_SUBPROCESS_SCRUB) {
    ...
    // secrets like INPUT_OPENSIN_API_KEY. No-op for vars that aren't action inputs.
    delete env[`INPUT_${k}`]
  }
  return env
}
