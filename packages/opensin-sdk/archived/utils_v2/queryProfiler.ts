 ...
 * pipeline from user input to first token arrival. Enable by setting OPENSIN_CODE_PROFILE_QUERY=1
 *
 ...
 * - query_client_creation_start/end: Creating OpenSIN client
...
const ENABLED = isEnvTruthy(process.env.OPENSIN_CODE_PROFILE_QUERY)
...
function getQueryProfileReport(): string {
  if (!ENABLED) {
    return 'Query profiling not enabled (set OPENSIN_CODE_PROFILE_QUERY=1)'
  }
