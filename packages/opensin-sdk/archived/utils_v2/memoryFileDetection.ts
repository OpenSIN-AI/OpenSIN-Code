...
/**
 * Detects if a file path is a session-related file under ~/.opensin.
...
/**
 * Check if a file is a OpenSIN-managed memory file (NOT user-managed instruction files).
 * Includes: auto-memory (memdir), agent memory, session memory/transcripts.
 * Excludes: OPENSIN.md, OPENSIN.local.md, .opensin/rules/*.md (user-managed).
 *
...
export function isMemoryDirectory(dirPath: string): boolean {
  ...
  // Check the auto-memory path override (OPENSIN_COWORK_MEMORY_PATH_OVERRIDE)
  if (isAutoMemoryEnabled()) {
  ...
  const configDirCmp = toComparable(getOpenSINConfigHomeDir())
...
// Excludes OPENSIN.md, OPENSIN.local.md, .opensin/rules/ (user-managed).
