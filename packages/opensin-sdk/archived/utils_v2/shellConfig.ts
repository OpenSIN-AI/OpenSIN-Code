 ...
 * Used for managing opensin aliases and PATH entries
 */
...
import { getLocalOpenSINPath } from './localInstaller.js'

export const OPENSIN_ALIAS_REGEX = /^\s*alias\s+opensin\s*=/
...
/**
 * Filter out installer-created opensin aliases from an array of lines
 * Only removes aliases pointing to $HOME/.opensin/local/opensin
 ...
 */
export function filterOpenSINAliases(lines: string[]): {
  filtered: string[]
  hadAlias: boolean
} {
  let hadAlias = false
  const filtered = lines.filter(line => {
    // Check if this is a opensin alias
    if (OPENSIN_ALIAS_REGEX.test(line)) {
      // Extract the alias target - handle spaces, quotes, and various formats
      // First try with quotes
      let match = line.match(/alias\s+opensin\s*=\s*["']([^"']+)["']/)
      if (!match) {
        // Try without quotes (capturing until end of line or comment)
        match = line.match(/alias\s+opensin\s*=\s*([^#\n]+)/)
      }
...
/**
 * Check if a opensin alias exists in any shell config file
 * Returns the alias target if found, null otherwise
 * @param options Optional overrides for testing (env, homedir)
 */
export async function findOpenSINAlias(
  options?: ShellConfigOptions,
): Promise<string | null> {
  ...
  for (const configPath of Object.values(configs)) {
    ...
    for (const line of lines) {
      if (OPENSIN_ALIAS_REGEX.test(line)) {
        ...
        const match = line.match(/alias\s+opensin=["']?([^"'\s]+)/)
        if (match && match[1]) {
          return match[1]
        }
      }
...
/**
 * Check if a opensin alias exists and points to a valid executable
 * Returns the alias target if valid, null otherwise
 * @param options Optional overrides for testing (env, homedir)
 */
export async function findValidOpenSINAlias(
  options?: ShellConfigOptions,
): Promise<string | null> {
  const aliasTarget = await findOpenSINAlias(options)
