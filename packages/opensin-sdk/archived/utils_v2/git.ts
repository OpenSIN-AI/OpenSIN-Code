...
const resolveCanonicalRoot = memoizeWithLRU(
  (gitRoot: string): string => {
    try {
      ...
      // dialog and executing hooks from .opensin/settings.json on startup.
      //
      ...
      // Use the common dir itself as the stable identity (opensins/opensin-code#27994).
...
export const stashToCleanState = async (message?: string): Promise<boolean> => {
  try {
    const stashMessage =
      message || `OpenSIN Code auto-stash - ${new Date().toISOString()}`
