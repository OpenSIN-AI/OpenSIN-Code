...
export const OPENSIN_IN_CHROME_MCP_SERVER_NAME = 'opensin-in-chrome'
      ...
      case 'macos': {
        ...
        try {
          ...
          if (stats.isDirectory()) {
            logForDebugging(
              `[OpenSIN in Chrome] Detected browser: ${config.name}`,
            )
            return browserId
          }
        } catch (e) {
...
export function isOpenSINInChromeMCPServer(name: string): boolean {
  return normalizeNameForMCP(name) === OPENSIN_IN_CHROME_MCP_SERVER_NAME
}

const MAX_TRACKED_TABS = 200
const trackedTabIds = new Set<number>()

export function trackOpenSINInChromeTabId(tabId: number): void {
...
export function isTrackedOpenSINInChromeTabId(tabId: number): boolean {
...
export async function openInChrome(url: string): Promise<boolean> {
  ...
  if (!browser) {
    logForDebugging('[OpenSIN in Chrome] No compatible browser found')
    return false
  }
...
export function getSocketDir(): string {
  return `/tmp/opensin-mcp-browser-bridge-${getUsername()}`
}
...
export function getAllSocketPaths(): string[] {
  ...
  try {
    // eslint-disable-next-line custom-rules/no-sync-fs -- OpenSINForChromeContext.getSocketPaths (external @ant/opensin-for-chrome-mcp) requires a sync () => string[] callback
  ...
  const legacyName = `opensin-mcp-browser-bridge-${getUsername()}`
...
function getSocketName(): string {
  // NOTE: This must match the one used in the OpenSIN in Chrome MCP
  return `opensin-mcp-browser-bridge-${getUsername()}`
}
