...
export const getGlobalOpenSINFile = memoize((): string => {
  // Legacy fallback for backwards compatibility
  if (
    getFsImplementation().existsSync(
      join(getOpenSINConfigHomeDir(), '.config.json'),
    )
  ) {
    return join(getOpenSINConfigHomeDir(), '.config.json')
  }

  const filename = `.opensin${fileSuffixForOauthConfig()}.json`
  return join(process.env.OPENSIN_CONFIG_DIR || homedir(), filename)
})
 ...
 * If OPENSIN_CODE_HOST_PLATFORM is set to a valid platform value, that overrides
...
export function getHostPlatformForAnalytics(): Platform {
  const override = process.env.OPENSIN_CODE_HOST_PLATFORM
