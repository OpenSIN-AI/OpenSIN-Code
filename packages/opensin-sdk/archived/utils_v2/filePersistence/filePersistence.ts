 ...
 * - Checks environment kind (OPENSIN_CODE_ENVIRONMENT_KIND)
 * - Retrieves session access token
 * - Requires OPENSIN_CODE_REMOTE_SESSION_ID for session ID
 *
...
export async function runFilePersistence(
  ...
  const sessionId = process.env.OPENSIN_CODE_REMOTE_SESSION_ID
  if (!sessionId) {
    logError(
      new Error(
        'File persistence enabled but OPENSIN_CODE_REMOTE_SESSION_ID is not set',
      ),
    )
    return null
  }
