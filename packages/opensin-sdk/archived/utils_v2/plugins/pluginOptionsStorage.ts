...
export function savePluginOptions(
  ...
  // pluginConfigs into ~/.opensin/settings.json. Same pattern exists in
...
export function deletePluginOptions(pluginId: string): void {
  ...
  // removal. Cast is deliberate (OPENSIN.md's 10% case): adding z.undefined()
...
/**
 * Substitute ${OPENSIN_PLUGIN_ROOT} and ${OPENSIN_PLUGIN_DATA} with their paths.
 * On Windows, normalizes backslashes to forward slashes so shell commands
 * don't interpret them as escape characters.
 *
 * ${OPENSIN_PLUGIN_ROOT} — version-scoped install dir (recreated on update)
 * ${OPENSIN_PLUGIN_DATA} — persistent state dir (survives updates)
 *
...
export function substitutePluginVariables(
  ...
  let out = value.replace(/\$\{OPENSIN_PLUGIN_ROOT\}/g, () =>
    normalize(plugin.path),
  )
  // source can be absent (e.g. hooks where pluginRoot is a skill root without
  // a plugin context). In that case ${OPENSIN_PLUGIN_DATA} is left literal.
  if (plugin.source) {
    const source = plugin.source
    out = out.replace(/\$\{OPENSIN_PLUGIN_DATA\}/g, () =>
