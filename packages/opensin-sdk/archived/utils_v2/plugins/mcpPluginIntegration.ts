 ...
 * Handles ${OPENSIN_PLUGIN_ROOT}, ${user_config.X}, and general ${VAR} substitution
...
export function resolvePluginMcpEnvironment(
  ...
  switch (config.type) {
    ...
    case 'stdio': {
      ...
      // Resolve environment variables and add OPENSIN_PLUGIN_ROOT / OPENSIN_PLUGIN_DATA
      const resolvedEnv: Record<string, string> = {
        OPENSIN_PLUGIN_ROOT: plugin.path,
        OPENSIN_PLUGIN_DATA: getPluginDataDir(plugin.source),
        ...(stdioConfig.env || {}),
      }
      for (const [key, value] of Object.entries(resolvedEnv)) {
        if (key !== 'OPENSIN_PLUGIN_ROOT' && key !== 'OPENSIN_PLUGIN_DATA') {
    ...
    // For other types (sse-ide, ws-ide, sdk, opensinai-proxy), pass through unchanged
    case 'sse-ide':
    case 'ws-ide':
    case 'sdk':
