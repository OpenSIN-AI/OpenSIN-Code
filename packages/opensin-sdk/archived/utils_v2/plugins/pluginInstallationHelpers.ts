 ...
 * 1. Caching a plugin to ~/.opensin/plugins/cache/
  ...
  if (cacheResult.path !== versionedPath) {
    ...
    if (isSubdirectory) {
      ...
      const tempPath = join(
        ...
        `.opensin-plugin-temp-${Date.now()}-${randomBytes(4).toString('hex')}`,
      )
...
export async function installPluginFromMarketplace({
  ...
  try {
    ...
    // non-official) because dbt external_opensin_code_plugin_installs.sql
