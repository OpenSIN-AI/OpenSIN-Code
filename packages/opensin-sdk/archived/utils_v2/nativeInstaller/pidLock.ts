 ...
 *
 * This module provides PID-based locking for running OpenSIN Code versions.
...
/**
 * Validate that a running process is actually a OpenSIN process
 * This helps mitigate PID reuse issues
 */
function isOpenSINProcess(pid: number, expectedExecPath: string): boolean {
  if (!isProcessRunning(pid)) {
    return false
  }

  // If the PID matches our current process, we know it's valid
  // This handles test environments where the command might not contain 'opensin'
  if (pid === process.pid) {
    return true
  }
  ...
  try {
    ...
    // Check if the command contains 'opensin' or the expected exec path
    const normalizedCommand = command.toLowerCase()
    const normalizedExecPath = expectedExecPath.toLowerCase()

    return (
      normalizedCommand.includes('opensin') ||
...
export function isLockActive(lockFilePath: string): boolean {
  ...
  // Secondary validation: is it actually a OpenSIN process?
  // This helps with PID reuse scenarios
  if (!isOpenSINProcess(pid, execPath)) {
    logForDebugging(
      `Lock PID ${pid} is running but does not appear to be OpenSIN - treating as stale`,
    )
    return false
  }
...
export async function tryAcquireLock(
  ...
  // validates it's actually a OpenSIN process (to handle PID reuse scenarios)
