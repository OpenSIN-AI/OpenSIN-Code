...
export async function readFile(
  ...
  try { stats = fs.statSync(resolvedPath); } catch {
    ...
    return { output: `Error: File not found: ${resolvedPath}${suggestion}`, isError: true, errorCode: 4 };
  }

  if (stats.isDirectory()) {
    return { output: `Error: Path is a directory, not a file: ${resolvedPath}\nUse Glob to list files in this directory.`, isError: true, errorCode: 5 };
  }

  if (stats.size > MAX_FILE_READ_SIZE) {
    return { output: `Error: File too large to read (${(stats.size / 1024 / 1024).toFixed(1)}MB). Maximum: 1MB.\nUse offset and limit parameters to read specific portions.`, isError: true, errorCode: 6 };
  }

  let buffer: Buffer;
  try { buffer = fs.readFileSync(resolvedPath); } catch (error) {
    return { output: `Error: Cannot read file: ${error instanceof Error ? error.message : String(error)}`, isError: true, errorCode: 7 };
  }

  if (buffer.length === 0) {
    return { output: '(File is empty)',
    isError: false, metadata: { filePath: resolvedPath, totalLines: 0, sizeBytes: 0 } };
  }

  if (isBinaryFile(buffer)) {
    const ext = path.extname(resolvedPath);
    return { output: `Error: Cannot read binary file (${ext || 'unknown'} format).`, isError: true, errorCode: 8 };
  }
