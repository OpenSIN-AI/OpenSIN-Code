...
export const readInputSchema: ToolDefinition['inputSchema'] = {
  ...
  properties: {
    ...
    limit: { type: 'number', description: 'The number of lines to read. Default: 2000', minimum: 1 },
  },
  required: ['file_path'],
...
export async function readFile(filePath: string, context: SecurityContext, options?: { offset?: number; limit?: number }): Promise<ToolResult> {
  ...
  return { content: [{ type: 'text', text: output }], metadata: { filePath: resolvedPath, startLine, endLine, totalLines, sizeBytes: buffer.length, encoding } };
}
