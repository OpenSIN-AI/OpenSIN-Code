...
export const grepInputSchema: ToolDefinition['inputSchema'] = {
  ...
  properties: {
    ...
    head_limit: { type: 'number', description: 'Limit output to first N results. Defaults to 250. Pass 0 for unlimited.' },
...
export async function grepSearch(pattern: string, context: SecurityContext, options?: { path?: string; glob?: string; outputMode?: 'content' | 'files_with_matches' | 'count'; caseInsensitive?: boolean; showLineNumbers?: boolean; headLimit?: number; offset?: number }): Promise<ToolResult> {
  ...
  if (outputMode === 'content') {
    ...
    return { content: [{ type: 'text', text: truncated ? output + '\n\n[Results truncated. Use offset=' + (offset + effectiveLimit) + ' for more.]' : output }], metadata: { mode: 'content', totalMatches: results.length, filesSearched: filesToSearch.length, filesWithMatches: filesWithMatches.size } };
  }
  if (outputMode === 'count') {
    ...
    return { content: [{ type: 'text', text: lines.join('\n') || 'No matches found' }], metadata: { mode: 'count', totalMatches, filesWithMatches: filesWithMatches.size, filesSearched: filesToSearch.length } };
  }
  ...
  return { content: [{ type: 'text', text: 'Found ' + sortedFiles.length + ' file' + (sortedFiles.length === 1 ? '' : 's') + '\n' + relFiles.join('\n') + (truncated ? '\n\n[Results truncated. Use offset=' + (offset + effectiveLimit) + ' for more.]' : '') }], metadata: { mode: 'files_with_matches', totalFiles: sortedFiles.length, filesSearched: filesToSearch.length } };
}
