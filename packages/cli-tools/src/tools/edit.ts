 ...
 * 
 * Performs targeted search-and-replace edits on files with
 * validation, stale-write detection, and diff output.
 */
...
export async function editFile(
  ...
  return {
    content: [{ type: 'text', text: `File edited successfully: ${resolvedPath}\n\n${diff}` }],
    metadata: { type: 'update', filePath: resolvedPath, occurrencesReplaced: replaceAll ? occurrences : 1, linesChanged },
  };
}
