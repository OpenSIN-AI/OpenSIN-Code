...
describe('BashTool', () => {
  ...
  it('handles errors', async () => {
    const r = await BashTool.execute({ command: 'false' });
    expect(r.isError).toBe(true);
  });
  it('blocks dangerous', async () => {
    const r = await BashTool.execute({ command: 'rm -rf /' });
    expect(r.isError).toBe(true);
  ...
  it('handles timeout', async () => {
    const r = await BashTool.execute({ command: 'sleep 10', timeout: 100 });
    expect(r.isError).toBe(true);
...
describe('ReadTool', () => {
  it('reads a file', async () => {
    const r = await ReadTool.execute({ file_path: path.join(testDir, 'hello.txt') });
    expect(r.isError).toBeFalsy();
    expect(r.content[0].text).toContain('line 1');
  });
  it('supports offset', async () => {
    const r = await ReadTool.execute({ file_path: path.join(testDir, 'hello.txt'), offset: 2 });
    expect(r.content[0].text).toContain('2: line 2');
    expect(r.content[0].text).not.toContain('1: line 1');
  });
  it('supports limit', async () => {
    const r = await ReadTool.execute({ file_path: path.join(testDir, 'hello.txt'), limit: 1 });
    expect(r.content[0].text).toContain('line 1');
    expect(r.content[0].text).not.toContain('line 2');
  });
  it('errors on nonexistent', async () => {
    const r = await ReadTool.execute({ file_path: '/nonexistent/file.txt' });
    expect(r.isError).toBe(true);
  });
  it('blocks sensitive paths', async () => {
    const r = await ReadTool.execute({ file_path: '/etc/passwd' });
    expect(r.isError).toBe(true);
  });
});
...
describe('WriteTool', () => {
  it('creates a file', async () => {
    ...
    const r = await WriteTool.execute({ file_path: fp, content: 'hello' });
    expect(r.isError).toBeFalsy();
  ...
  it('overwrites a file', async () => {
    ...
    const r = await WriteTool.execute({ file_path: fp, content: 'new' });
    expect(r.isError).toBeFalsy();
    expect(fs.readFileSync(fp, 'utf8')).toBe('new');
  });
  it('creates parent dirs', async () => {
    const fp = path.join(testDir, 'deep', 'nested', 'file.txt');
    const r = await WriteTool.execute({ file_path: fp, content: 'nested' });
    expect(r.isError).toBeFalsy();
    expect(fs.readFileSync(fp, 'utf8')).toBe('nested');
  });
  it('blocks sensitive paths', async () => {
    const r = await WriteTool.execute({ file_path: '/etc/shadow', content: 'x' });
    expect(r.isError).toBe(true);
  });
});
...
describe('EditTool', () => {
  it('edits a file', async () => {
    ...
    const r = await EditTool.execute({ file_path: fp, old_string: 'hello', new_string: 'goodbye' });
    expect(r.isError).toBeFalsy();
    expect(fs.readFileSync(fp, 'utf8')).toBe('goodbye world');
  });
  it('errors when not found', async () => {
    const fp = path.join(testDir, 'edit.txt');
    const r = await EditTool.execute({ file_path: fp, old_string: 'nonexistent', new_string: 'x' });
    expect(r.isError).toBe(true);
  });
  it('errors when identical', async () => {
    const fp = path.join(testDir, 'edit.txt');
    const r = await EditTool.execute({ file_path: fp, old_string: 'hello', new_string: 'hello' });
    expect(r.isError).toBe(true);
  });
  it('replaces all', async () => {
    const fp = path.join(testDir, 'multi.txt');
    fs.writeFileSync(fp, 'foo foo foo');
    const r = await EditTool.execute({ file_path: fp, old_string: 'foo', new_string: 'bar', replace_all: true });
    expect(r.isError).toBeFalsy();
  ...
  it('errors on multiple without replace_all', async () => {
    const fp = path.join(testDir, 'multi.txt');
    fs.writeFileSync(fp, 'foo foo foo');
    const r = await EditTool.execute({ file_path: fp, old_string: 'foo', new_string: 'bar' });
    expect(r.isError).toBe(true);
  });
});
...
describe('GrepTool', () => {
  it('finds matches', async () => {
    const r = await GrepTool.execute({ pattern: 'hello', path: testDir });
    expect(r.isError).toBeFalsy();
    expect(r.content[0].text).toContain('Found');
    expect(r.content[0].text).toContain('app.ts');
  });
  it('shows content', async () => {
    const r = await GrepTool.execute({ pattern: 'hello', path: testDir, output_mode: 'content' });
    expect(r.isError).toBeFalsy();
    expect(r.content[0].text).toContain('hello');
  });
  it('no matches', async () => {
    const r = await GrepTool.execute({ pattern: 'zzzzz', path: testDir });
...
describe('GlobTool', () => {
  it('finds files', async () => {
    const r = await GlobTool.execute({ pattern: '*.ts', path: path.join(testDir, 'src') });
    expect(r.isError).toBeFalsy();
    expect(r.content[0].text).toContain('app.ts');
    expect(r.content[0].text).toContain('utils.ts');
  });
  it('finds json files', async () => {
    const r = await GlobTool.execute({ pattern: '*.json', path: testDir });
    expect(r.isError).toBeFalsy();
    expect(r.content[0].text).toContain('package.json');
  });
  it('no matches', async () => {
    const r = await GlobTool.execute({ pattern: '**/*.xyz', path: testDir });
    expect(r.content[0].text).toContain('No files found');
  });
  it('empty pattern error', async () => {
    const r = await GlobTool.execute({ pattern: '' });
    expect(r.isError).toBe(true);
  });
});
