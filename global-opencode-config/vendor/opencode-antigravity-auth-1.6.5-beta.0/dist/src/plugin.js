...
import { startAntigravityDebugRequest, logAntigravityDebugResponse, logAccountContext, logRateLimitEvent, logRateLimitSnapshot, logResponseBody, logModelFamily, isDebugEnabled, getLogFilePath, initializeDebug, debugLogToFile, } from "./plugin/debug";
...
function summarizeRefreshToken(refreshToken) {
    return refreshToken ? `${refreshToken.length}:${refreshToken.slice(-8)}` : "none";
}
function getAuthRefreshSignature(auth) {
    if (!auth?.refresh) {
        return "";
    }
...
function summarizeAccountIdentityState(accountManager) {
    return accountManager.getAccounts().map((account) => {
        const refreshToken = account.parts?.refreshToken ?? "";
        const projectId = account.parts?.managedProjectId ?? account.parts?.projectId ?? "";
        return `${account.index}:${account.email ?? ""}:${summarizeRefreshToken(refreshToken)}:${projectId}:${account.enabled !== false}`;
    }).join("|");
}
function snapshotAccountBlockState(accountManager) {
    return accountManager.getAccounts().map((account) => ({
        identity: `${account.email ?? ""}:${summarizeRefreshToken(account.parts?.refreshToken ?? "")}:${account.parts?.managedProjectId ?? account.parts?.projectId ?? ""}`,
        rateLimitKeys: Object.keys(account.rateLimitResetTimes ?? {}).sort(),
        coolingDownUntil: account.coolingDownUntil ?? 0,
    }));
}
function hasBlockStateRelaxed(previousState, currentState) {
    if (!previousState.length || !currentState.length) {
        return false;
    }
    const currentByIdentity = new Map(currentState.map((entry) => [entry.identity, entry]));
    for (const previous of previousState) {
        const current = currentByIdentity.get(previous.identity);
        if (!current) {
            continue;
        }
        if (previous.coolingDownUntil && !current.coolingDownUntil) {
            return true;
        }
        for (const key of previous.rateLimitKeys) {
            if (!current.rateLimitKeys.includes(key)) {
                return true;
            }
        }
    }
                ...
                return {
                    ...
                    async fetch(input, init) {
                        ...
                        const reloadProviderState = async (reason) => {
                            ...
                            debugLogToFile(`[reload-provider-state] reason=${reason} accounts=${accountManager.getAccountCount()}`);
                            return true;
                        };
                        let _lastSeenRefreshSignature = getAuthRefreshSignature(latestAuth);
                        let _lastSeenAccountIdentityState = summarizeAccountIdentityState(accountManager);
                        let _lastSeenAccountBlockState = snapshotAccountBlockState(accountManager);
                        while (true) {
                            // Check for abort at the start of each iteration
                            checkAborted();
                            const currentAuth = await getAuth();
                            accountManager = await AccountManager.loadFromDisk(currentAuth);
                            activeAccountManager = accountManager;
                            const _currentAccounts = accountManager.getAccounts();
                            const _currentRefreshSignature = getAuthRefreshSignature(currentAuth);
                            const _currentAccountIdentityState = summarizeAccountIdentityState(accountManager);
                            const _currentAccountBlockState = snapshotAccountBlockState(accountManager);
                            const _currentAuthParts = currentAuth?.refresh
                                ? parseRefreshParts(currentAuth.refresh)
                                : { refreshToken: "", projectId: "", managedProjectId: "" };
                            const _firstAccount = _currentAccounts[0];
                            const _authChanged = !!(_lastSeenRefreshSignature && _currentRefreshSignature && _currentRefreshSignature !== _lastSeenRefreshSignature);
                            const _accountIdentityChanged = !!(_lastSeenAccountIdentityState && _currentAccountIdentityState && _currentAccountIdentityState !== _lastSeenAccountIdentityState);
                            const _blockStateRelaxed = hasBlockStateRelaxed(_lastSeenAccountBlockState, _currentAccountBlockState);
                            const _authStorageMismatch = !!(_firstAccount && _currentAuthParts.refreshToken && (_firstAccount.parts.refreshToken !== _currentAuthParts.refreshToken || ((_firstAccount.parts.managedProjectId ?? _firstAccount.parts.projectId ?? "") !== (_currentAuthParts.managedProjectId ?? _currentAuthParts.projectId ?? ""))));
                            const _authSummary = `${summarizeRefreshToken(_currentAuthParts.refreshToken)}:${_currentAuthParts.managedProjectId ?? _currentAuthParts.projectId ?? ""}`;
                            const _storageSummary = _currentAccounts.map((account) => `${account.index}:${account.email ?? ""}:${summarizeRefreshToken(account.parts?.refreshToken ?? "")}:${account.parts?.managedProjectId ?? account.parts?.projectId ?? ""}`).join("|");
                            debugLogToFile(`[loop] family=${family} auth=${_authSummary} storage=${_storageSummary}`);
                            if (_authChanged || _accountIdentityChanged || _authStorageMismatch || _blockStateRelaxed) {
                                rateLimitStateByAccountQuota.clear();
                                emptyResponseAttempts.clear();
                                accountFailureState.clear();
                                for (const _acc of _currentAccounts) {
                                    if (_acc && _acc.rateLimitResetTimes) {
                                        const _keys = Object.keys(_acc.rateLimitResetTimes);
                                        for (const _k of _keys)
                                            delete _acc.rateLimitResetTimes[_k];
                                    }
                                    if (_acc) {
                                        _acc.coolingDownUntil = undefined;
                                        _acc.cooldownReason = undefined;
                                    }
                                }
                                rateLimitToastShown = false;
                                softQuotaToastShown = false;
                                await accountManager.saveToDisk();
                                pushDebug(`hot-reload authChanged=${_authChanged} accountIdentityChanged=${_accountIdentityChanged} authStorageMismatch=${_authStorageMismatch} blockStateRelaxed=${_blockStateRelaxed} auth=${_authSummary} storage=${_storageSummary}`);
                                debugLogToFile(`[hot-reload] flushed stale state authChanged=${_authChanged} accountIdentityChanged=${_accountIdentityChanged} authStorageMismatch=${_authStorageMismatch} blockStateRelaxed=${_blockStateRelaxed} auth=${_authSummary} storage=${_storageSummary}`);
                            }
                            _lastSeenRefreshSignature = _currentRefreshSignature;
                            _lastSeenAccountIdentityState = _currentAccountIdentityState;
                            _lastSeenAccountBlockState = _currentAccountBlockState;
                            ...
                            if (accountCount === 0) {
                                ...
                                const pollWaitMs = Math.max(1000, (config.max_rate_limit_wait_seconds ?? 10) * 1000);
                                if (!rateLimitToastShown) {
                                    await showToast(`No Antigravity accounts available. Waiting ${Math.ceil(pollWaitMs / 1000)}s for fresh credentials...`, "warning");
                                    rateLimitToastShown = true;
                                }
                                await sleep(pollWaitMs, abortSignal);
                                continue;
                            }
                            ...
                            if (!account) {
                                ...
                                debugLogToFile(`[all-rate-limited] family=${family} accounts=${accountCount} waitMs=${waitMs} auth=${_authSummary} storage=${_storageSummary}`);
                                if (isDebugEnabled()) {
                                ...
                                if (maxWaitMs > 0 && waitMs > maxWaitMs) {
                                    ...
                                    if (!rateLimitToastShown) {
                                        await showToast(`All ${accountCount} account(s) rate-limited for ${family}. Waiting ${Math.ceil(maxWaitMs / 1000)}s for fresh credentials...`, "warning");
                                        rateLimitToastShown = true;
                                    }
                                    await sleep(maxWaitMs, abortSignal);
                                    continue;
                                }
                                if (!rateLimitToastShown) {
