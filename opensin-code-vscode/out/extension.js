...
function activate(context) {
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider('opensin-code.chatView', provider));
    ...
    // Initialize defaults
    provider.setCurrentMode(modes_1.AGENT_MODES[1]); // Default: Code mode
    provider.setCurrentModel('opencode/qwen3.6-plus-free');
    modeStatusBar.text = `$(symbol-misc) ${modes_1.AGENT_MODES[1].name}`;
    ...
    context.subscriptions.push(vscode.commands.registerCommand('opensin-code.selectMode', async () => {
        const items = modes_1.AGENT_MODES.map(m => ({
            label: `${m.icon} ${m.name}`,
...
class KairosViewProvider {
    ...
    addFileToContext(path) { this.contextFiles.push(path); }
