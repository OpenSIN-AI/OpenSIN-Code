...
exports.MarketplacePanel = void 0;
...
class MarketplacePanel {
    ...
    show() {
        ...
        this.panel = vscode.window.createWebviewPanel('sincode.marketplace', '🤖 SIN Agent Marketplace', vscode.ViewColumn.One, { enableScripts: true, retainContextWhenHidden: true });
...
<body>
    ...
    <div class="categories">
        ${[...new Set(this.agents.map(a => a.category))].map(cat => `<button class="cat-btn" onclick="filter('${cat}')">${cat}</button>`).join('')}
