...
class LspProvider {
    ...
    async getSymbolsInFile() {
        ...
        return new Promise((resolve) => {
            const process = (0, child_process_1.spawn)('simone-mcp', ['symbols', '--file', editor.document.fileName, '--format', 'json']);
            let output = '';
            process.stdout.on('data', (data) => output += data.toString());
            process.on('close', () => {
                try {
                    const symbols = JSON.parse(output);
                    resolve(Array.isArray(symbols) ? symbols.map((s) => ({
                        name: s.name,
                        kind: s.kind,
                        file: editor.document.fileName,
                        line: s.line || 0
