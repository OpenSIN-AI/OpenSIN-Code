...
import { InlineChatProvider } from './inlineChat';
import { SINCodeActionProvider, fixError, refactorSelection, explainCode, generateTests } from './codeActions';
import { MarketplacePanel } from './agentMarketplace';
...
export function activate(context: vscode.ExtensionContext) {
    ...
    const provider = new SINCodeViewProvider(context.extensionUri, lspProvider, swarmCoordinator, buddySystem, memoryConsolidation);
    ...
    provider.setCurrentMode(AGENT_MODES[1]);
    ...
    context.subscriptions.push(vscode.commands.registerCommand('sincode.selectMode', async () => {
        const items = AGENT_MODES.map(m => ({ label: `${m.icon} ${m.name}`, description: m.description, mode: m }));
    ...
    context.subscriptions.push(vscode.commands.registerCommand('sincode.swarmDispatch', async () => {
        const agentItems = AVAILABLE_AGENTS.map(a => ({ label: `${a.icon} ${a.name}`, description: a.description, agent: a.id }));
        ...
        vscode.window.withProgress({
            ...
            try {
                await swarmCoordinator.dispatchTask(selectedAgent.agent, prompt);
    ...
    context.subscriptions.push(vscode.commands.registerCommand('sincode.buddyInfo', () => {
        ...
        vscode.window.showInformationMessage(`BUDDY Status:\nMood: ${state.mood}\nLevel: ${state.level}\nXP: ${state.xp}/${state.level * 100}\nLast: ${state.lastAction}`);
    }));
    ...
    context.subscriptions.push(vscode.commands.registerCommand('sincode.start', () => {
        vscode.commands.executeCommand('workbench.view.extension.sincode-sidebar');
    }));

    // ===== Phase 3: Inline Chat =====
    const inlineController = new InlineChatProvider();
    context.subscriptions.push(
        vscode.languages.registerInlineCompletionItemProvider({ pattern: '**' }, inlineController)
    );
    ...
    context.subscriptions.push(
        vscode.languages.registerCodeActionsProvider(
            { pattern: '**/*' },
            new SINCodeActionProvider(),
            { providedCodeActionKinds: SINCodeActionProvider.providedCodeActionKinds }
        )
    );
    context.subscriptions.push(vscode.commands.registerCommand('sincode.fixError', fixError));
    context.subscriptions.push(vscode.commands.registerCommand('sincode.refactorSelection', refactorSelection));
    context.subscriptions.push(vscode.commands.registerCommand('sincode.explainCode', explainCode));
    context.subscriptions.push(vscode.commands.registerCommand('sincode.generateTests', generateTests));

    // ===== Phase 3: Agent Marketplace =====
    const marketplace = new MarketplacePanel();
    context.subscriptions.push(vscode.commands.registerCommand('sincode.openMarketplace', () => {
        marketplace.show();
    }));
}

class SINCodeViewProvider implements vscode.WebviewViewProvider {
    ...
    public addFileToContext(p: string) { this.contextFiles.push(p); }
    public notifyModeChange(mode: AgentMode, message?: string) {
        if (this.view) { this.view.webview.postMessage({ type: 'mode-change', mode: mode.name, message }); }
    }
...
<div class="header"><span>SIN Code</span><span class="mode-badge" id="mode-badge">Code</span></div>
...
<div class="swarm-panel"><h3>🐝 Swarm Coordinator</h3><button class="btn" onclick="vscode.postMessage({type:'command',command:'swarmDispatch'})">Dispatch Agent</button></div>
<div class="swarm-panel" style="margin-top:8px"><h3>🤖 Agent Marketplace</h3><button class="btn" onclick="vscode.postMessage({type:'openMarketplace'})">Open Marketplace</button></div>
<script>
const vscode=acquireVsCodeApi();const chat=document.getElementById('chat');const input=document.getElementById('input');const status=document.getElementById('status');const sendBtn=document.getElementById('send-btn');const cancelBtn=document.getElementById('cancel-btn');const modeBadge=document.getElementById('mode-badge');let currentAiMessage=null;
function sendMessage(){if(input.value.trim()){const val=input.value.trim();const div=document.createElement('div');div.className='msg-user';div.textContent=val;chat.appendChild(div);chat.scrollTop=chat.scrollHeight;vscode.postMessage({type:'prompt',value:val});input.value='';currentAiMessage=null;sendBtn.style.display='none';cancelBtn.style.display='block';}}
input.addEventListener('keypress',(e)=>{if(e.key==='Enter')sendMessage();});sendBtn.addEventListener('click',sendMessage);cancelBtn.addEventListener('click',()=>{vscode.postMessage({type:'cancel'});sendBtn.style.display='block';cancelBtn.style.display='none';});
window.addEventListener('message',event=>{const message=event.data;switch(message.type){case'stream':if(!currentAiMessage){currentAiMessage=document.createElement('div');currentAiMessage.className='msg-ai';chat.appendChild(currentAiMessage);}currentAiMessage.textContent+=message.text;chat.scrollTop=chat.scrollHeight;break;case'status':status.textContent=message.message;if(message.message==='Done.'||message.message==='Cancelled.'){setTimeout(()=>status.textContent='',2000);currentAiMessage=null;sendBtn.style.display='block';cancelBtn.style.display='none';}break;case'error':status.textContent='Error: '+message.message;status.style.color='var(--vscode-errorForeground)';sendBtn.style.display='block';cancelBtn.style.display='none';break;case'mode-change':modeBadge.textContent=message.mode;if(message.message){const sysDiv=document.createElement('div');sysDiv.className='msg-ai';sysDiv.style.fontStyle='italic';sysDiv.style.opacity='0.7';sysDiv.textContent=message.message;chat.appendChild(sysDiv);chat.scrollTop=chat.scrollHeight;}break;case'openMarketplace':vscode.postMessage({type:'command',command:'openMarketplace'});break;}});
</script></body></html>`;
    }
}
