...
export class ZeusDispatcher {
  ...
  async dispatch(description: string, prompt: string, agent = 'explore'): Promise<string | undefined> {
    try {
      const { stdout } = await execAsync(
        `opencode run --agent ${agent} --description "${description}" --background --prompt "${prompt}"`
      );
  ...
  async healthCheck() {
    ...
    for (const [id, task] of this.tasks) {
      try {
        const { stdout } = await execAsync(`opencode background-output --task-id ${task.bg_task_id}`);
