...
2. Corrections or reversions (user fixing Sin's actions)
...
For each confirmed behavior, create a `.sin/hookify.{rule-name}.local.md` file:
...
{Message to show Sin when rule triggers}
```
...
**IMPORTANT**: Rule files must be created in the current working directory's `.sin/` folder, NOT the plugin directory.

Use the current working directory (where OpenSIN Code was started) as the base path.

1. Check if `.sin/` directory exists in current working directory
   - If not, create it first with: `mkdir -p .sin`

2. Use Write tool to create each `.sin/hookify.{name}.local.md` file
   - Use relative path from current working directory: `.sin/hookify.{name}.local.md`
   - The path should resolve to the project's .sin directory, not the plugin's

3. Show user what was created:
   ```
   Created 3 hookify rules:
   - .sin/hookify.dangerous-rm.local.md
   - .sin/hookify.console-log.local.md
...
4. Create `.sin/hookify.dangerous-rm.local.md`:
   ```markdown
   ---
...
- **File location**: Create files in project's `.sin/` directory (current working directory), NOT the plugin's .sin/
...
2. Ensure `.sin/` directory exists (create with mkdir if needed)
3. Use absolute path if needed: `{cwd}/.sin/hookify.{name}.local.md`
4. Verify file was created with Glob or ls

**If rule doesn't trigger after creation:**
1. Verify file is in project `.sin/` not plugin `.sin/`
