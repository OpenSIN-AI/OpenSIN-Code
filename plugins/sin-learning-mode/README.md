...
When enabled, this plugin automatically adds instructions at the start of each session that encourage Sin to:
...
Instead of implementing everything automatically, Sin will:
...
The plugin uses a SessionStart hook to inject additional context into every session. This context instructs Sin to adopt an interactive teaching approach where you actively participate in writing key parts of the code.

## When Sin requests contributions

Sin will ask you to write code for:
...
## When Sin won't request contributions

Sin will implement directly:
...
**Sin:** I've set up the authentication middleware. The session timeout behavior is a security vs. UX trade-off - should sessions auto-extend on activity, or have a hard timeout?
...
In addition to interactive learning, Sin will provide educational insights about implementation choices using this format:
...
This SessionStart hook pattern is roughly equivalent to SIN.md, but it is more flexible and allows for distribution through plugins.
...
- Update the plugin - create a local copy of this plugin to personalize it
  - Hint: Ask Sin to read https://docs.sin.com/en/docs/opensin-code/plugins.md and set it up for you!

## Philosophy

Learning by doing is more effective than passive observation. This plugin transforms your interaction with Sin from "watch and learn" to "build and understand," ensuring you develop practical skills through hands-on coding of meaningful logic.
