...
3. Create directory structure using bash:
   ```bash
   mkdir -p plugin-name/.sin-plugin
...
6. Create .gitignore if needed (for .sin/*.local.md, etc.)
...
2. For each command:
   ...
   - Write instructions FOR Sin (not TO user)
...
2. Create .mcp.json configuration with:
   ...
   - Command and args (with ${SIN_PLUGIN_ROOT})
...
3. Create example .sin/plugin-name.local.md file (as documentation)
4. Implement settings reading in hooks/commands as needed
5. Add to .gitignore: `.sin/*.local.md`
...
1. **Installation instructions**:
   - Show user how to test locally:
     ...
     ```
   - Or copy to `.sin-plugin/` for project testing
...
3. **Testing recommendations**:
   ...
   - For hooks: Use `sin --debug` to see hook execution
...
- **Apply best practices**:
  ...
  - Commands written FOR Sin
  - Strong trigger phrases
  - ${SIN_PLUGIN_ROOT} for portability
