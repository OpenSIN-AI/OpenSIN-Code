...
4. **Plugin Settings** - Configuration patterns using .sin/plugin-name.local.md files
...
**Trigger phrases:** "create a hook", "add a PreToolUse hook", "validate tool use", "implement prompt-based hooks", "${SIN_PLUGIN_ROOT}", "block dangerous commands"
...
- ${SIN_PLUGIN_ROOT} for portable paths
...
- Environment variable expansion (${SIN_PLUGIN_ROOT}, user vars)
...
- ${SIN_PLUGIN_ROOT} usage throughout
...
**What it covers:**
- .sin/plugin-name.local.md pattern for configuration
...
✅ **Portability**
- Use ${SIN_PLUGIN_ROOT} everywhere
- Relative paths only
...
Daisy Hollman (daisy@opensin-ai.com)
