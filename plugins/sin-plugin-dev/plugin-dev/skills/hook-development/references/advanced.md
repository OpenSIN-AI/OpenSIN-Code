  ...
  "PreToolUse": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "bash ${SIN_PLUGIN_ROOT}/scripts/quick-check.sh",
          "timeout": 5
        },
        {
          "type": "prompt",
...
if [ -f ".sin-hooks-config.json" ]; then
  strict_mode=$(jq -r '.strict_mode' .sin-hooks-config.json)
...
# Append to audit log
echo "$timestamp | $USER | $tool_name | $input" >> ~/.sin/audit.log
...
export SIN_PROJECT_DIR="/tmp/test-project"
export SIN_PLUGIN_ROOT="$(pwd)"
mkdir -p "$SIN_PROJECT_DIR"
