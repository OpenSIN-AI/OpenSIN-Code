...
- [SIN_PLUGIN_ROOT Environment Variable](#sin_plugin_root-environment-variable)
...
## SIN_PLUGIN_ROOT Environment Variable

### Purpose

`${SIN_PLUGIN_ROOT}` is a special environment variable available in plugin commands that resolves to the absolute path of the plugin directory.
...
Run analysis: !`node ${SIN_PLUGIN_ROOT}/scripts/analyze.js`

Read template: @${SIN_PLUGIN_ROOT}/templates/report.md
```
...
Lint results: !`node ${SIN_PLUGIN_ROOT}/bin/lint.js $1`
...
Configuration: @${SIN_PLUGIN_ROOT}/config/deploy-config.json
...
Use this template: @${SIN_PLUGIN_ROOT}/templates/api-report.md
...
Step 1 - Prepare: !`bash ${SIN_PLUGIN_ROOT}/scripts/prepare.sh $1`
Step 2 - Config: @${SIN_PLUGIN_ROOT}/config/$1.json
Step 3 - Execute: !`${SIN_PLUGIN_ROOT}/bin/execute $1`
...
1. **Always use for plugin-internal paths:**
   ```markdown
   # Good
   @${SIN_PLUGIN_ROOT}/templates/foo.md
...
2. **Validate file existence:**
   ...
   !`test -f ${SIN_PLUGIN_ROOT}/config.json && echo "exists" || echo "missing"`

   If config exists, load it: @${SIN_PLUGIN_ROOT}/config.json
...
4. **Combine with arguments:**
   ```markdown
   Run: !`${SIN_PLUGIN_ROOT}/bin/process.sh $1 $2`
   ```
...
- Verify syntax is exact: `${SIN_PLUGIN_ROOT}`
...
Load configuration: @${SIN_PLUGIN_ROOT}/deploy-config.json
...
Template: @${SIN_PLUGIN_ROOT}/templates/component-docs.md
...
Validate: !`bash ${SIN_PLUGIN_ROOT}/scripts/validate.sh`
...
Environment config: @${SIN_PLUGIN_ROOT}/config/$1.json
...
Cache directory: ${SIN_PLUGIN_ROOT}/cache/

Analyze @$1 and save results to cache:
!`mkdir -p ${SIN_PLUGIN_ROOT}/cache && date > ${SIN_PLUGIN_ROOT}/cache/last-run.txt`
...
- Sin will automatically use Task tool to launch agent
...
- Mention skill by name to hint Sin should invoke it
...
- Config exists: !`test -f ${SIN_PLUGIN_ROOT}/config.json && echo "✓" || echo "✗"`
- Scripts exist: !`test -d ${SIN_PLUGIN_ROOT}/scripts && echo "✓" || echo "✗"`
- Tools available: !`test -x ${SIN_PLUGIN_ROOT}/bin/analyze && echo "✓" || echo "✗"`
...
Try processing: !`node ${SIN_PLUGIN_ROOT}/scripts/process.js $1 2>&1 || echo "ERROR: $?"`
...
1. **Use ${SIN_PLUGIN_ROOT} for all plugin-internal paths**
