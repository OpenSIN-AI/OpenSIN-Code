...
- Can use any tool Sin determines is necessary
...
Sin automatically structures tool calls based on schema:

```typescript
// Sin generates this internally
{
...
```markdown
Steps:
1. Make parallel calls (Sin handles this automatically):
...
2. **Install plugin locally** in `.sin-plugin/`
3. **Verify tools available** with `/mcp`
4. **Test command** that uses tools
5. **Check debug output**: `sin --debug`
