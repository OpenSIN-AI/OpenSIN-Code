...
- Plugin-specific features (${SIN_PLUGIN_ROOT}, plugin patterns)
...
- **plugin-features-reference.md**: Plugin-specific command features
  - Plugin command discovery and organization
  - ${SIN_PLUGIN_ROOT} environment variable usage
  - Plugin command patterns (configuration, template, multi-script)
...
Sin loads references and examples as needed based on task.
...
- **Project**: `.sin/commands/` (shared with team)
- **Personal**: `~/.sin/commands/` (your commands)
...
- ✓ Plugin command patterns (${SIN_PLUGIN_ROOT}, discovery, organization)
