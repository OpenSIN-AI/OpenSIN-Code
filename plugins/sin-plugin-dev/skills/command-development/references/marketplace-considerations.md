...
if [ ! -f ".sin/command-initialized" ]; then
...
Checking for user config: .sin/plugin-name.local.md

if [ -f ".sin/plugin-name.local.md" ]; then
  # Parse YAML frontmatter for settings
  VERBOSE=$(grep "^verbose:" .sin/plugin-name.local.md | cut -d: -f2 | tr -d ' ')
  COLOR=$(grep "^color:" .sin/plugin-name.local.md | cut -d: -f2 | tr -d ' ')
  MAX_RESULTS=$(grep "^max_results:" .sin/plugin-name.local.md | cut -d: -f2 | tr -d ' ')

  echo "✓ Using user configuration"
else
  echo "Using default configuration"
  echo "Create .sin/plugin-name.local.md to customize"
fi
...
Or set in .sin/plugin-name.local.md:
\`\`\`yaml
---
format: yaml
output: custom.txt
...
if [ -f ".sin/operation-completed.flag" ]; then
  ℹ️  Operation already completed

  Completed at: $(cat .sin/operation-completed.flag)

  To re-run:
  1. Remove flag: rm .sin/operation-completed.flag
  2. Run command again
...
Marking complete...
echo "$(date)" > .sin/operation-completed.flag
```
