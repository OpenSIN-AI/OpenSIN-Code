...
head -n 20 .sin/commands/my-command.md | grep -A 10 "^---"

# Check for closing frontmatter marker
head -n 20 .sin/commands/my-command.md | grep -c "^---" # Should be 2

# Verify file has .md extension
ls .sin/commands/*.md

# Check file is in correct location
test -f .sin/commands/my-command.md && echo "Found" || echo "Missing"
```
...
cat > .sin/commands/test-bash.md << 'EOF'
---
...
cat > .sin/commands/test-forbidden.md << 'EOF'
---
...
# Command: .sin/commands/risky-operation.md
...
COMMANDS_CHANGED=$(git diff --cached --name-only | grep "\.sin/commands/.*\.md")
  ...
  test:
    ...
    steps:
      ...
      - name: Check for TODOs
        run: |
          if grep -r "TODO" .sin/commands/; then
...
```bash
# Check file location
ls -la .sin/commands/my-command.md

# Check permissions
chmod 644 .sin/commands/my-command.md

# Check syntax
head -n 20 .sin/commands/my-command.md
...
```bash
# Verify syntax
grep '\$1' .sin/commands/my-command.md
grep '\$ARGUMENTS' .sin/commands/my-command.md

# Test with simple command first
echo "Test: \$1 and \$2" > .sin/commands/test-args.md
```
...
```bash
# Check allowed-tools
grep "allowed-tools" .sin/commands/my-command.md

# Verify command syntax
grep '!\`' .sin/commands/my-command.md
