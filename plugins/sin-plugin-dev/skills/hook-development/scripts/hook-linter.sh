...
check_script() {
  ...
  if grep -E '^[^#]*/home/|^[^#]*/usr/|^[^#]*/opt/' "$script" | grep -q .; then
    ...
    echo "   Use \$SIN_PROJECT_DIR or \$SIN_PLUGIN_ROOT"
    ((warnings++))
  fi

  # Check 8: Uses SIN_PLUGIN_ROOT
  if ! grep -q "SIN_PLUGIN_ROOT\|SIN_PROJECT_DIR" "$script"; then
    echo "💡 Tip: Use \$SIN_PLUGIN_ROOT for plugin-relative paths"
  fi
