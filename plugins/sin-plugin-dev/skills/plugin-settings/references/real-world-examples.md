...
Detailed analysis of how production plugins use the `.sin/plugin-name.local.md` pattern.
...
SWARM_STATE_FILE=".sin/multi-agent-swarm.local.md"
...
```bash
cat > "$WORKTREE_PATH/.sin/multi-agent-swarm.local.md" <<EOF
---
...
sed "s/^pr_number: .*/pr_number: $PR_NUM/" \
  ".sin/multi-agent-swarm.local.md" > temp.md
mv temp.md ".sin/multi-agent-swarm.local.md"
```
...
Document any changes needed in SIN.md.
```
...
**Purpose:** Prevent session exit and loop Sin's output back as input
...
RALPH_STATE_FILE=".sin/ralph-loop.local.md"
...
# Create state file
cat > ".sin/ralph-loop.local.md" <<EOF
---
iteration: 1
...
echo "Ralph loop initialized: .sin/ralph-loop.local.md"
```
...
|---------|-------------------|--------------|
| **File** | `.sin/multi-agent-swarm.local.md` | `.sin/ralph-loop.local.md` |
...
```bash
# BAD
FILE="/Users/alice/.sin/my-plugin.local.md"

# GOOD
FILE=".sin/my-plugin.local.md"
```
...
The `.sin/plugin-name.local.md` pattern provides:
